...
begin
  inherited;
  BtnFrame(Canvas, Rect(job1.Left, job1.Top, job7.Left + job7.Width, job1.Top + job1.Height),
    MainTexture);
  BtnFrame(Canvas, Rect(job3.Left, job3.Top, job9.Left + job9.Width, job3.Top + job3.Height),
    MainTexture);
...
begin
  ...
  job3.Visible := not TypeChanged and (Terrain[TerrType].ClearTerrain >= 0) and
    ...
    (LastJob = jClear);
  job6.Visible := not TypeChanged and (Terrain[TerrType].AfforestTerrain >= 0) or
    (LastJob = jAfforest);
...
procedure TEnhanceDlg.FormKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
begin
