...
{$IFDEF LOADPERF}time_total, time_total0, time_x0, time_x1, time_a,
  time_b, time_c: int64;
{$ENDIF}
...
var
  p: integer;
begin
  if (Imp[Wonder].Expiration >= 0) and (GWonder[woEiffel].EffectiveOwner <>
    GWonder[Wonder].EffectiveOwner) then
    for p := 0 to nPl - 1 do // check if already expired
      if (1 shl p and GAlive <> 0) and (RW[p].Tech[Imp[Wonder].Expiration] >=
        tsApplicable) then
      begin
...
procedure dxdy(Loc0, Loc1: integer; var dx, dy: integer);
begin
  dx := ((Loc1 mod lx * 2 + Loc1 div lx and 1) - (Loc0 mod lx * 2 + Loc0 div lx and 1) +
    3 * lx) mod (2 * lx) - lx;
...
var
  ...
  begin
    Result := 0;
    for i := 1 to d do
      Result := Result + sin(f1[i] * ((x * 2 + y and 1) * sa[i] + y *
        1.5 * ca[i])) * f2[i];
...
begin
  ...
  for y := 1 to ly - 2 do
    ...
    begin
      ...
      if (y and 1 = 0) and (y - 1 >= 1) and (RealMap[lx - 1 + lx * (y - 1)] and
...
var
  i, j, Cnt, x, y, dx, dy, Loc0, Loc1, xworst, yworst, totalrare,
  RareMaxWater, RareType, iBest, jbest, MinDist, xBlock, yBlock, V8: integer;
      ...
      for y := 0 to 4 do
        ...
        begin
          if (AreaCount[(x + 1) mod 8, y] > 0) and
            (Continent[RareByArea[x, y]] = Continent[RareByArea[(x + 1) mod 8, y]]) then
          begin
          ...
          begin
            if (AreaCount[x, y + 1] > 0) and
              (Continent[RareByArea[x, y]] = Continent[RareByArea[x, y + 1]]) then
            begin
              Inc(RareAdjacent[x, y]);
              Inc(RareAdjacent[x, y + 1]);
            end;
            if (AreaCount[(x + 1) mod 8, y + 1] > 0) and
              (Continent[RareByArea[x, y]] =
              Continent[RareByArea[(x + 1) mod 8, y + 1]]) then
            begin
              Inc(RareAdjacent[x, y]);
            ...
            if (AreaCount[(x + 7) mod 8, y + 1] > 0) and
              (Continent[RareByArea[x, y]] =
              Continent[RareByArea[(x + 7) mod 8, y + 1]]) then
            begin
              Inc(RareAdjacent[x, y]);
...
begin
  ...
  begin
    ...
    begin
      Loc1 := Radius[V21];
      if (Loc1 >= 0) and (Loc1 < MapSize) and (RealMap[Loc1] and
        fTerrain >= fGrass) and (RealMap[Loc1] and fTerrain <> fArctic) then
...
const
  ...
  begin
    ...
    if (Result > 1) and ((Result >= MinRivLen) or (RealMap[Loc1] and
      fTerrain >= fGrass)) then
    begin
      Loc := Loc1;
      while Loc <> Loc0 do
...
begin
  ...
  for Loc0 := 0 to MapSize - 1 do
    ...
    begin
      ...
      begin
        Loc1 := Adjacent[V8];
        if (Loc1 < 0) or (Loc1 >= MapSize) or (RealMap[Loc1] and
          fTerrain < fGrass) or (RealMap[Loc1] and fTerrain = fArctic) then
  ...
  begin
    ...
    if RealMap[x + lx * (ly - 2)] >= fGrass then
      RealMap[x + lx * (ly - 2)] :=
        RealMap[x + lx * (ly - 2)] and not fTerrain or fTundra;
...
var
  ...
  begin
    ...
    if RealMap[Loc] and fDeadLands <> 0 then
      ...
      case CountGood of
        cgBest:
          Result := (RealMap[Loc] and fTerrain in
            [fGrass, fPrairie, fTundra, fSwamp, fForest]) and Odd(
            (lymax + xLoc - yLoc shr 1) shr 1 + xLoc + (yLoc + 1) shr 1);
        cgFlat:
          Result := (RealMap[Loc] and fTerrain in [fGrass, fPrairie,
            fTundra, fSwamp, fForest]);
        cgLand:
...
begin
  ...
  repeat
    ...
    begin
      if ((Loc >= 4 * lx) and (Loc < MapSize - 4 * lx) or (CountGood >= cgLand)) and
        IsGoodTile(Loc) then
      begin
        c := nsc;
  ...
  begin
    RealMap[StartLoc0[p1]] := RealMap[StartLoc0[p1]] and not
      (fTerrain or fSpecial) or fGrass or fSpecial1;
    CntGood := 1;
    CntGoodGrass := 1;
    ...
    for V21 := 1 to 26 do
      ...
      begin
        ...
        if (Loc1 >= 0) and (Loc1 < MapSize) and (RealMap[Loc1] and fDeadLands = 0) then
          ...
          else if (RealMap[Loc1] and fTerrain in [fPrairie, fTundra, fSwamp]) and
            (random(2) = 0) then
    ...
    for V21 := 1 to 26 do
      ...
      begin
        Loc1 := Radius[V21];
        if (Loc1 >= 0) and (Loc1 < MapSize) and (RealMap[Loc1] and
          (fTerrain or fSpecial) = fGrass or fSpecial1) then
        begin
          IrrLoc[nIrrLoc] := Loc1;
          Inc(nIrrLoc);
        end;
      ...
      begin
        ...
        for p2 := 0 to p1 - 1 do
          if (1 shl p2 and GAlive <> 0) and (StartLoc2[p2] = Loc1) then
            Loc1 := -1;
        if (Loc1 < 0) or (Loc1 >= MapSize) or (RealMap[Loc1] and
          fTerrain in [fOcean, fShore, fDesert, fArctic, fMountains]) or
...
begin
  ...
  if (Terrain[NewTile and fTerrain].MineEff = 0) and (NewTile and fTerImp = tiMine) then
    NewTile := NewTile and not fTerImp;
...
begin
  ...
  begin
    ...
    if Tile and fCity <> 0 then
      Trade := 0
    else if (cix >= 0) and (RW[p].City[cix].Built[imCourt] +
      RW[p].City[cix].Built[imPalace] = 0) then
...
var
  i, uix, cix, TerrOwner, TerrOwnerTreaty, Strength, Bonus, Cnt,
  pFoundCity, cixFoundCity, MinLevel, Loc1, V8: integer;
  ...
  begin
    ...
    begin
      ...
      begin // set fPeace flag?
        ...
        begin
          ...
          if 1 shl TerrOwnerTreaty and (1 shl trPeace or 1 shl
            TrFriendlyContact) <> 0 then
      ...
      if Occupant[Loc] >= 0 then
        ...
        begin
          ...
          begin
            ...
            if (pTell = p) and ((Tile and fCity = 0) or
              (1 shl pTell and GAI <> 0)) then
              Result := True;
          end
          else
    ...
    if euix >= -1 then
      Map[Loc] := Map[Loc] and not (fUnit or fCity or fOwned or fOwnZoCUnit) or
        (Tile and $07FFFFFF or AddFlags) and (fUnit or fCity or fOwned or fOwnZoCUnit)
    else
    begin
        ...
        with RW[pTell].Un[uix] do
          ...
          begin
            ...
            if (PModel.Kind = mkDiplomat) or (PModel.Cap[mcSpy] > 0) then
              ...
              (RealMap[Loc] and fTerImp = tiBase) or (PModel.Cap[mcAcademy] > 0) then
              Discover21(Loc, p, lObserveUnhidden, False, PModel.Domain = dGround)
            else
              Discover9(Loc, p, lObserveUnhidden, False, PModel.Domain = dGround);
          end;
...
begin
  ...
  for p1 := -1 to nPl - 1 do
    PeacePlayer[p1] := (p1 >= 0) and (p1 <> p) and (1 shl p1 and GAlive <> 0) and
      (RW[p].Treaty[p1] in [trPeace, trFriendlyContact]);
  for Loc := 0 to MapSize - 1 do
...
var
  i, r, Loc, Loc1, dx, dy, p1, p2, cix, NewDist, dxMax, dyMax, OldOwner,
  V8, NewOwner: integer;
  Adjacent: TVicinity8Loc;
  ...
  begin // update area only
        ...
        begin
          ...
          begin
            ...
            if NewOwner <> OldOwner then
              if AtPeace[NewOwner, OldOwner] and not
                ((OldOwner = PlayerLosingCity) and
    ...
    begin
      ...
      if (NewOwner <> OldOwner) and (not AtPeace[NewOwner, OldOwner] or
        ((OldOwner = PlayerLosingCity) and (StolenDist[Loc] > CountryRadius))) then
      begin
...
{$IFOPT O-}
  if OriginLoc <> -2 then
    CheckBorders(-2);
{$ENDIF}//check: single pass should do!
end; //CheckBorders
  ...
  if Mode > moLoading_Fast then
    for p1 := 0 to nPl - 1 do
      if (1 shl p1 and (GAlive or GWatching) <> 0) and
        ...
        RW[p1].Map[Loc] := RW[p1].Map[Loc] and not
          (fTerrain or fSpecial or fTerImp or fRoad or fRR or fCanal or fPoll) or
          RealMap[Loc] and (fTerrain or fSpecial or fTerImp or fRoad or
          fRR or fCanal or fPoll);
end; //CompleteJob
...
begin
  for Loc := 0 to MapSize - 1 do
    ...
    begin
      ...
      RW[pTarget].Map[Loc] := RW[pTarget].Map[Loc] and
        fInEnemyZoC // always preserve this flag!
        or Tile and not (fUnit or fHiddenUnit or fStealthUnit or
        fObserved or fSpiedOut or fOwned or fInEnemyZoC or fOwnZoCUnit or
        fPeace or fGrWall);
...
begin
  ...
  case Price and opMask of
    ...
    opMoney: // + value
      if (Price - opMoney <= MaxMoneyPrice) and (RW[pSender].Money >=
        Price - opMoney) then
      begin
        if Execute then
        begin
  ...
  begin
    for p1 := 0 to nPl - 1 do
      if (p1 = pWith) or DecreaseCredibility and (p1 <> p) and
        (RW[pWith].Treaty[p1] = trAlliance) and (RW[p].Treaty[p1] >= trPeace) then
      begin
...
begin
  ...
  case Mission of
    ...
    begin
      for p1 := 0 to nPl - 1 do
        if (p1 <> p) and (p1 <> pCity) and (RW[pCity].EnemyReport[p1] <> nil) then
        begin
...
begin
  ...
  case Command of
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('IntTellAboutNation P%d+P%d', [Player, Subject]);
{$ENDIF}
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('IntHaveContact P%d+P%d', [Player, Subject]);
{$ENDIF}
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('IntCancelTreaty P%d with P%d', [Player, Subject]);
{$ENDIF}
...
(*  sIntChoosePeace:
    ...
    {$IFDEF TEXTLOG}
      CmdInfo := Format('IntTellAboutModel P%d about P%d Mod%d', [p1, Player, Subject]);
{$ENDIF}
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('IntDiscoverZOC P%d Loc%d', [Player, integer(Data)]);
{$ENDIF}
    ...
    sIntExpandTerritory:
      ...
      begin
      {$IFDEF TEXTLOG}
        CmdInfo := Format('IntExpandTerritory P%d Loc%d',
...
{$ENDIF}
    ...
    sIntBuyMaterial:
      ...
      begin
      {$IFDEF TEXTLOG}
        CmdInfo := Format('IntBuyMaterial P%d Loc%d Cost%d',
          [Player, Loc, integer(Data)]);
{$ENDIF}
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('IntPayPrices P%d+P%d', [Player, Subject]);
{$ENDIF}
    ...
    sIntSetModelStatus: if ProcessClientData[Player] then
      begin
    {$IFDEF TEXTLOG}
        CmdInfo := Format('IntSetModelStatus P%d', [Player]);
{$ENDIF}
    ...
    sIntSetUnitStatus: if ProcessClientData[Player] then
      begin
    {$IFDEF TEXTLOG}
        CmdInfo := Format('IntSetUnitStatus P%d', [Player]);
{$ENDIF}
    ...
    sIntSetCityStatus: if ProcessClientData[Player] then
      begin
    {$IFDEF TEXTLOG}
        CmdInfo := Format('IntSetCityStatus P%d', [Player]);
{$ENDIF}
    ...
    sIntSetECityStatus: if ProcessClientData[Player] then
      begin
    {$IFDEF TEXTLOG}
        CmdInfo := Format('IntSetECityStatus P%d', [Player]);
{$ENDIF}
