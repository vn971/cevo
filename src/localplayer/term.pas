...
type
  TMainScreen = class(TDrawDlg)
    ...
    procedure PanelBoxMouseMove(Sender: TObject; Shift: TShiftState; x, y: integer);
...
var
  ...
  ScienceSum, TaxSum, SoundPreloadDone, MarkCityLoc,
  MovieSpeed: integer;
...
procedure ChooseModelPicture(p, mix, code, Hash, Turn: integer; ForceNew, final: boolean);
var
  i: integer;
...
begin
  Result := 'ATTACK_' + char(48 + code div 100 mod 10) + char(48 + code div 10 mod
    10) + char(48 + code mod 10);
end;
...
begin
  ...
  with MyUn[uix] do
    if (Status and usToldNoReturn <> 0) and ((MyMap[Loc] and fCity <> 0) or
      (MyMap[Loc] and fTerImp = tiBase) or (Master >= 0)) then
...
begin
  ...
  if Tile and (fTerrain or fSpecial) = fGrass or fSpecial1 then
    ...
    if Tile and fTerrain < fForest then
      ...
      TSprite(0, yyt, 8, 2 + (Tile and fTerrain - fForest) * 2 + Tile and fSpecial shr 5)
...
begin
  ...
  begin
    ...
    TerrainBtn.Visible := (Tile and fCity = 0) and (MyUn[UnFocus].Master < 0) and
  ...
  begin
    ...
    begin
      ...
      for i := 0 to nPl - 1 do
        if Tribe[i] <> nil then
        begin
          TestColorDistance := abs(integer(UnusedTribeFiles.Objects[j]) shr
            16 and $FF - Tribe[i].Color shr 16 and $FF) + abs(
            integer(UnusedTribeFiles.Objects[j]) shr 8 and $FF - Tribe[i].Color shr
            8 and $FF) * 3 + abs(integer(UnusedTribeFiles.Objects[j]) and
...
const
  ...
  const
    nAdvBookIcon = 16;
    AdvBookIcon: array[0..nAdvBookIcon - 1] of record
        Adv, Icon: integer
      end
    =
...
{    if MyRO.Happened and phGameEnd<>0 then
    ...
    if (ToldSlavery < 0) or ((ToldSlavery = 1) <>
      (MyRO.Wonder[woPyramids].EffectiveOwner >= 0)) then
    begin
    ...
    begin
      for cix := 0 to MyRO.nCity - 1 do
        if (MyCity[cix].Loc >= 0) and ((MyRO.Turn = 0) or
          (MyCity[cix].Flags and chFounded <> 0)) then
          MyCity[cix].Status := MyCity[cix].Status and not
            csResourceWeightsMask or (3 shl 4); // new city, set to maximum growth
    end;
    ...
    begin
      ...
      if (ClientMode = cContinue) and (DipMem[me].SentCommand and $FF0F = scContact) then
        ...
        if MyRO.Treaty[DipMem[me].pContact] >= trPeace then
          ...
          SoundMessage(Tribe[DipMem[me].pContact].TPhrase('FRREJECTED'), 'NEGO_REJECTED');
      ...
      if not supervising then
      begin
        if RoughCredibility(MyRO.Credibility) <> RoughCredibility(
    ...
    begin // tell about new techs
      for ad := 0 to nAdv - 1 do
        if (MyRO.TestFlags and tfAllTechs = 0) and
          ((MyRO.Tech[ad] >= tsApplicable) <> (MyData.ToldTech[ad] >= tsApplicable)) or
          (ad in FutureTech) and (MyRO.Tech[ad] <> MyData.ToldTech[ad]) then
          with MessgExDlg do
          begin
            ...
            begin
              ...
              begin
                ...
                if (Size >= NeedAqueductSize) and
                  ...
                  (Size >= NeedSewerSize) and (MyRO.Tech[Imp[imSewer].Preq] < tsApplicable) then
            ...
            begin
              ...
              begin {continue multi-turn goto}
                ...
                if Status shr 16 = $7FFF then
                  ...
                  MoveResult := GetMoveAdvice(UnFocus, Status shr 16, MoveAdviceData);
                ...
                begin // !!! Shinkansen
                  ...
                  begin
                    ...
                    if (MyMap[Loc1] and
                      (fCity or fOwned) = fCity) // don't capture cities during auto move
                    ...
                    begin
                      if (Loc1 = MoveAdviceData.ToLoc) or
                        ...
                        (MyMap[dLoc(Loc, MoveAdviceData.dx[i], MoveAdviceData.dy[i])] and fCity <> 0) then
                  ...
                  Stop := not ok or (Loc = MoveAdviceData.ToLoc) or
                    (MoveAdviceData.ToLoc = maNextCity) and (MyMap[Loc] and fCity <> 0);
                end
                else
                begin
                  MoveResult := eOK;
      ...
      begin
        ...
        begin
          if AILogo[pLogo] <> nil then
            BitBltUgly(Canvas, (xRightPanel + 10) - (16 + 64),
              ClientHeight - PanelHeight, 64, 64,
              AILogo[pLogo].Canvas, 0, 0, SRCCOPY);
        end;
      end;
    end;
...
*)
      ...
      begin
        ...
        begin
          ...
          if idle then
            ...
            CurrentMoveInfo.DoShow := not (mAlNoMoves.Checked or
              mAlEffectiveMovesOnly.Checked)
          else
            CurrentMoveInfo.DoShow := not (mEnNoMoves.Checked or mEnAttacks.Checked);
        end;
    ...
    cShowMoving, cShowCapturing:
      if (idle and (NewPlayer = me) or not idle and not skipped and
        (TShowMove(Data).emix <> $FFFF)) and not ((GameMode = cMovie) and (MovieSpeed = 4)) then
      begin
        assert(NewPlayer = me);
        if not idle or (GameMode = cMovie) then
          Application.ProcessMessages;
            ...
            begin
              ...
              begin
                ...
                if (MyRO.Wonder[woShinkansen].EffectiveOwner = me) and
                  (Domain = dGround) and (MyMap[FromLoc] and
                  (fRR or fCity) <> 0) and (MyMap[ToLoc] and
                  (fRR or fCity) <> 0) and (Flags and umPlaneUnloading = 0) then
                  AnimationSpeed := 4;
          ...
          begin
            ...
            if GameMode = cMovie then
              ...
              CurrentMoveInfo.DoShow := not mAlNoMoves.Checked and
                not (mAlEffectiveMovesOnly.Checked and (Command <> cShowCapturing))
            else
              CurrentMoveInfo.DoShow := not mEnNoMoves.Checked and not
                (mEnAttacks.Checked and (Command <> cShowCapturing));
            if CurrentMoveInfo.DoShow then
            begin
              ...
              begin // allied unit -- make discovered land visible
                ...
                with MyRO.EnemyModel[emix] do
                  if (Kind = mkDiplomat) or (Domain = dAir) or
                    (ATrans_Fuel > 0) or
                    (Cap and (1 shl (mcRadar - mcFirstNonCap) or 1 shl (mcAcademy - mcFirstNonCap)) <>
                    0) or (MyMap[ToLoc] and fTerrain = fMountains) or
                    (MyMap[ToLoc] and fTerImp = tiFort) or
                    (MyMap[ToLoc] and fTerImp = tiBase) then
                    CurrentMoveInfo.AfterMovePaintRadius := 2
                  else
...
begin
  ...
  DefaultOptionChecked := 1 shl 1 + 1 shl 7 + 1 shl 10 + 1 shl 12 + 1 shl
...
begin
  ...
  begin
    ...
    for uix := 0 to MyRO.nUn - 1 do
      with MyUn[uix] do
        if (Loc >= 0) and (MyModel[mix].Domain = dAir) and
          (Status and usToldNoReturn = 0) and (Master < 0) and (MyMap[Loc] and fCity = 0) and
    ...
    if not supervising and (MyRO.TestFlags and tfImmImprove = 0) and
      (MyRO.Government <> gAnarchy) and (MyRO.Money + TaxSum < 0) and (MyRO.TaxRate < 100) then
      // low funds!
      with MessgExDlg do
      begin
...
begin
  ...
  begin
    if xMap < 0 then
      BitBltUgly(Canvas, MapOffset, TopBarHeight, Width + xMap, Height + yMap,
        Buffer.Canvas, -xMap, -yMap, SRCCOPY)
    else
      BitBltUgly(Canvas, xMap + MapOffset, TopBarHeight, Width, Height + yMap,
        Buffer.Canvas, 0, -yMap, SRCCOPY);
  end
  else
  begin
    if xMap < 0 then
      BitBltUgly(Canvas, MapOffset, TopBarHeight + yMap, Width + xMap, Height,
        Buffer.Canvas, -xMap, 0, SRCCOPY)
    else
      BitBltUgly(Canvas, xMap + MapOffset, TopBarHeight + yMap, Width, Height,
        Buffer.Canvas, 0, 0, SRCCOPY);
  end;
end;
...
begin
  ...
  if xw - xwd > G.lx div 2 then
    ...
    xwd := xwd - G.lx;
  if not MapValid or (xw - xwd > MapWidth div (xxt * 2)) or (xwd - xw > MapWidth div (xxt * 2)) or
    (yw - ywd > MapHeight div yyt) or (ywd - yw > MapHeight div yyt) then
  begin
    offscreen.Canvas.Font.Assign(UniFont[ftSmall]);
...
begin
  BitBltUgly(Panel.Canvas, xMini + 2, yMini + 2, G.lx * 2, G.ly, Mini.Canvas, 0, 0, SRCCOPY);
  if MarkCityLoc >= 0 then
    ...
    Frame(Panel.Canvas, xMini + 2 + G.lx - MapWidth div (xxt * 2), yMini + 2 + yw,
      xMini + 1 + G.lx + MapWidth div (xxt * 2),
...
var
  ...
  xTreasurySection, xResearchSection, JobFocus, TrueMoney, TrueResearch: integer;
  Tile: cardinal;
  s: string;
  unx: TUn;
  UnitInfo: TUnitInfo;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ImpImage(Panel.Canvas, ClientWidth - xPalace, yPalace, 21, -1, GameMode <> cMovie
          {(GameMode<>cMovie) and (MyRO.Government<>gAnarchy)});
    end;
      ...
      if UnFocus >= 0 then
        ...
        begin {display info about selected unit}
          ...
          begin
            ...
            LoweredTextOut(Panel.Canvas, -1, MainTexture, xMidPanel + 7 + xUnitText + 18, yTroop + 13, s);
        end;
...
{      j:=(i-MyRO.Research-1) div ScienceSum +1;
  ...
  begin
    ...
    RisedTextOut(TopBar.Canvas, 40 +
      (xTreasurySection - 40 - BiColorTextWidth(TopBar.Canvas, s)) div 2, 6, s);
    TopBar.Canvas.Font.Assign(UniFont[ftNormal]);
  end;
  ...
  begin
    ...
    begin
      ...
      if (MyUn[uix].Loc >= 0) and (MyUn[uix].Job = jNone) and
        (MyUn[uix].Status and (usStay or usRecover or usWaiting) = usWaiting) and
        (not GotoOnly or (MyUn[uix].Status and usGoto <> 0)) then
        if NearLoc < 0 then
        begin
          NewFocus := uix;
          Break;
...
begin
    ...
    begin
      ...
      if ssCtrl in Shift then // toggle special resource
        case MyMap[MouseLoc] and fTerrain of
          ...
          fGrass, fArctic: EditTileData.NewTile :=
              MyMap[MouseLoc] and not fSpecial or
        ...
        if MyMap[MouseLoc] and (fDeadLands or fModern) = BrushType and
          ...
          EditTileData.NewTile := MyMap[MouseLoc] and not (fDeadLands or fModern) or BrushType
        ...
        if MyMap[MouseLoc] and (fPrefStartPos or fStartPos) = BrushType and
          ...
          EditTileData.NewTile := MyMap[MouseLoc] and not
            (fPrefStartPos or fStartPos) or BrushType
      else
      ...
      PaintLoc(MouseLoc, 2);
      MiniPaint;
      BitBltUgly(Panel.Canvas, xMini + 2, yMini + 2, G.lx * 2, G.ly, Mini.Canvas,
        0, 0, SRCCOPY);
      if ywmax <= 0 then
        ...
        Frame(Panel.Canvas, xMini + 2 + G.lx - MapWidth div (2 * xxt), yMini + 2 + yw,
          ...
          begin // rotate
            ...
            begin
              if (MyUn[uix].Loc = MouseLoc) and (MyUn[uix].Job = jNone) and
                (MyUn[uix].Status and (usStay or usRecover or usEnhance or usWaiting) = usWaiting) then
                break;
              Dec(i);
  ...
  begin
    ...
    begin
        ...
        begin
          ...
          for emix := MyRO.nEnemyModel - 1 downto 0 do
            if (MyRO.EnemyModel[emix].Owner = p1) and
              (Server(sCreateUnit - sExecute + p1 shl 4, me, MyRO.EnemyModel[emix].mix, MouseLoc) >=
              rExecuted) then
            begin
      ...
      if FullScreen then
        ...
        EditPopup.Popup(Left + x + 4, Top + y + GetSystemMetrics(SM_CYCAPTION) + 4);
    end
  ...
  begin
    DestinationMarkON := False;
    PaintDestination;
    MyUn[UnFocus].Status := MyUn[UnFocus].Status and ($FFFF - usStay - usRecover - usGoto) or
      usEnhance;
    uix := UnFocus;
  ...
  else if (ClientMode <= cContinue) and (Button = mbRight) and (ssShift in Shift) and
    (UnFocus >= 0) and (MyMap[MouseLoc] and (fUnit or fOwned) = fUnit) then
...
begin
  ...
  begin // attack -- search enemy unit
    if (MyModel[MyUn[UnFocus].mix].Attack = 0) and not
      ((MyModel[MyUn[UnFocus].mix].Cap[mcBombs] > 0) and
  ...
  begin
    case Result of
      ...
      eZOC: ContextMessage(Phrases.Lookup('ZOC'), 'NOMOVE_ZOC', hkText,
      ...
      eTreaty:
        if MyMap[ToLoc] and (fUnit or fOwned) <> fUnit then {no enemy unit -- move}
          ...
          SoundMessage(Tribe[MyRO.EnemyUn[euix].Owner].TPhrase(
  ...
  if ((Result = eWon) or (Result = eLost) or (Result = eBloody)) and
    (MyUn[UnFocus].Movement < 100) and (MyModel[MyUn[UnFocus].mix].Cap[mcWill] = 0) then
  begin
  ...
  begin
      ...
      case Result of
        ...
        begin
          ...
          begin
            ...
            if CityCaptured and (MyRO.Government in
              [gRepublic, gDemocracy, gFuture]) then
    ...
    {if MyRO.Treaty[Defender]=trCeaseFire then
      if SimpleQuery(mkYesNo,Phrases.Lookup('FRCANCELQUERY_CEASEFIRE'),
        ...
        exit;}
      if (Options and muNoSuicideCheck = 0) and (Result and rUnitRemoved <> 0) and
        (Result <> eMissionDone) then
      begin // suicide query
        with MyUn[UnFocus], BattleDlg.Forecast do
        begin
          pAtt := me;
  ...
  begin
    if (UnFocus >= 0) and ((Result = eNoTime_Move) or UnitExhausted(UnFocus) or
      (MyUn[UnFocus].Master >= 0) or (MyModel[MyUn[UnFocus].mix].Domain = dAir) and
      ((MyMap[MyUn[UnFocus].Loc] and fCity <> 0) {aircrafts stop in cities} or
      (MyMap[MyUn[UnFocus].Loc] and fTerImp = tiBase))) then
    begin
...
begin
  ...
  begin
    ...
    while abs(((xFromLoc - xw1 + G.lx) * 2 + yFromLoc and 1 + 1) * xxt * 2 + dx * xxt - MapWidth) < abs(
        ((xFromLoc - xw1) * 2 + yFromLoc and 1 + 1) * xxt * 2 + dx * xxt - MapWidth) do
      Dec(xw1, G.lx);
    ...
    begin
      BitBltUgly(Buffer.Canvas, 0, 0, xRange, yRange,
        offscreen.Canvas, xMin, yMin, SRCCOPY);
      if Step1 <> Step0 then
      begin
  ...
  if Restore then
  begin
    BitBltUgly(Buffer.Canvas, 0, 0, xRange, yRange, offscreen.Canvas, xMin,
      yMin, SRCCOPY);
    PaintBufferToScreen(xMin, yMin, xRange, yRange);
  end;
  BlinkTime := -1;
...
begin
  ...
  begin
    ...
    case StopReason of
      ...
      Dead: if UnFocus < 0 then
          NextUnit(UnStartLoc, False);
      else
      begin // multi-turn goto
        if Loc = maNextCity then
          MyUn[uix].Status := MyUn[uix].Status and ($FFFF - usStay - usRecover) or
            usGoto + $7FFF shl 16
        else
          MyUn[uix].Status := MyUn[uix].Status and ($FFFF - usStay - usRecover) or usGoto + Loc shl 16;
...
begin
  ...
  begin
    if (x >= xMini + 2) and (y >= yMini + 2) and (x < xMini + 2 + 2 * G.lx) and (y < yMini + 2 + G.ly) then
      ...
      (y >= yPalace) and (x < ClientWidth - xPalace + xSizeBig) and (y < yPalace + ySizeBig) then
    begin
      InitPopup(StatPopup);
      if FullScreen then
...
(*  else if (x>=xAdvisor-3) and (y>=yAdvisor-3)
    ...
    AdviceBtnClick*)
    else if (x >= xTroop + 1) and (y >= yTroop + 1) and (x < xTroop + TrRow * TrPitch) and
      (y <= yTroop + 55) then
    begin
        ...
        begin
          ...
          begin
            if ssShift in Shift then
              ...
              (x - xTroop - 1 - i * TrPitch >= 60 - 20) and (y >= yTroop + 35) and
...
var
  i: integer;
begin
  if supervising and (G.RO[0].Turn > 0) and ((p = 0) or (1 shl p and G.RO[0].Alive <> 0)) then
  begin
    for i := 0 to Screen.FormCount - 1 do
      if Screen.Forms[i].Visible and (Screen.Forms[i] is TBufferedDrawDlg) then
        Screen.Forms[i].Close; // close windows
...
procedure TMainScreen.FormKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
...
procedure TMainScreen.MenuClick(Sender: TObject);
  ...
  begin
    ...
    begin
      ...
      Movement0 := Movement;
      if j0 < 0 then
        Result := ProcessEnhancement(UnFocus, MyData.EnhancementJobs) // terrain enhancement
      else
    ...
    case Result of
      ...
      eTreaty: SoundMessage(Tribe[MyRO.Territory[Loc0]].TPhrase(
          'PEACE_NOWORK'), 'NOMOVE_TREATY');
      else
        if Result < rExecuted then
          Play('INVALID')
    end;
...
var
  ...
  AltGovs, govermentChanged: boolean;
  QueryText: string;
...
begin
      ...
      if MyRO.nCity > 0 then
        for i := 28 to nImp - 1 do
          if (i <> imTrGoods) and (Imp[i].Kind = ikCommon) and
            (Imp[i].Preq <> preNA) and ((Imp[i].Preq = preNone) or
  ...
  begin
    ...
    mScienceStat.Enabled := not NoSuper or (MyRO.ResearchTech >= 0) or
      (MyRO.Happened and phTech <> 0) or (MyRO.Happened and
      phGameEnd <> 0) // no researchtech in case just completed
...
{  mWonders.Enabled:= false;
  ...
  begin
    ...
    extended := extended and ((mox.Kind = mkSettler) or (mox.Kind = mkSlaves) and
      (MyRO.Wonder[woPyramids].EffectiveOwner >= 0)) and (MyUn[UnFocus].Master < 0) and
      (Tile and fDeadLands = 0);
    if (mox.Kind = mkFreight) and (Tile and fCity <> 0) and not
      Phrases2FallenBackToEnglish or
      (Server(sRemoveUnit - sExecute, me, UnFocus, nil^) = eUtilized) then
    begin
    ...
    mRecover.Visible := (MyUn[UnFocus].Health < 100) and (Tile and fTerrain >= fGrass) and
      ((MyRO.Wonder[woGardens].EffectiveOwner = me) or (Tile and fTerrain <> fArctic) and
      (Tile and fTerrain <> fDesert)) and not ((mox.Domain = dAir) and
      (Tile and fCity = 0) and (Tile and fTerImp <> tiBase));
    mStay.Visible := not ((mox.Domain = dAir) and (Tile and fCity = 0) and
      (Tile and fTerImp <> tiBase));
    mCity.Visible := extended and (mox.Kind = mkSettler) or (Tile and fCity <> 0) and
      ((mox.Kind in [mkSettler, mkSlaves]) or (MyUn[UnFocus].Flags and unConscripts <> 0));
  ...
  begin
    ...
    extended := extended and ((mox.Kind = mkSettler) or (mox.Kind = mkSlaves) and
      (MyRO.Wonder[woPyramids].EffectiveOwner >= 0)) and (MyUn[UnFocus].Master < 0);
    if extended then
    begin
...
begin
  if Sender = UnitBtn then
    ...
    {if Sender=TerrainBtn then} Popup := TerrainPopup;
  InitPopup(Popup);
  if FullScreen then
...
begin
  ...
  begin
    ...
    begin
      ...
      BitBltUgly(Buffer.Canvas, 0, 0, G.lx * 2, G.ly, Mini.Canvas, 0, 0, SRCCOPY);
      if ywmax <= 0 then
      ...
      BitBltUgly(Panel.Canvas, xMini + 2, yMini + 2, G.lx * 2, G.ly, Buffer.Canvas,
        0, 0, SRCCOPY);
      MainOffscreenPaint;
...
begin
  Result := (y >= TopBarHeight + MapHeight) or (y >= ClientHeight - PanelHeight) and
    ...
    begin
      ...
      begin
        ...
        if FullScreen then
          ...
          GamePopup.Popup(Left + 4, Top + GetSystemMetrics(SM_CYCAPTION) + 4 + TopBarHeight - 1);
      end;
    end
...
procedure TMainScreen.FormMouseMove(Sender: TObject; Shift: TShiftState; x, y: integer);
begin
  if idle then
...
begin
  ...
  if (MapOffset > 0) or (MapOffset + MapWidth < ClientWidth) then
    ...
    begin // pillarbox, make left and right border black
      ...
      if xRightPanel < MapOffset + MapWidth then
        FillRect(Rect(MapOffset + MapWidth, TopBarHeight, ClientWidth, TopBarHeight +
          MapHeight - overlap))
      else
      begin
  ...
  BitBltUgly(Canvas, MapOffset, TopBarHeight, MapWidth, MapHeight - overlap,
    offscreen.Canvas,
    0, 0, SRCCOPY);
  BitBltUgly(Canvas, 0, 0, ClientWidth, TopBarHeight, TopBar.Canvas,
    0, 0, SRCCOPY);
  if xMidPanel > MapOffset then
    BitBltUgly(Canvas, xMidPanel, TopBarHeight + MapHeight - overlap,
      ClientWidth div 2 - xMidPanel, overlap,
      offscreen.Canvas, xMidPanel - MapOffset, MapHeight - overlap, SRCCOPY)
  else
    BitBltUgly(Canvas, MapOffset, TopBarHeight + MapHeight - overlap,
      ClientWidth div 2 - MapOffset, overlap,
      offscreen.Canvas, 0, MapHeight - overlap, SRCCOPY);
  if xRightPanel < MapOffset + MapWidth then
    BitBltUgly(Canvas, ClientWidth div 2, TopBarHeight + MapHeight - overlap,
      xRightPanel - ClientWidth div 2, overlap,
      offscreen.Canvas, ClientWidth div 2 - MapOffset, MapHeight - overlap, SRCCOPY)
  else
    BitBltUgly(Canvas, ClientWidth div 2, TopBarHeight + MapHeight - overlap,
      MapOffset + MapWidth - ClientWidth div 2, overlap,
      offscreen.Canvas, ClientWidth div 2 - MapOffset, MapHeight - overlap, SRCCOPY);
  BitBltUgly(Canvas, 0, TopBarHeight + MapHeight - overlap, xMidPanel, overlap,
    Panel.Canvas, 0, 0, SRCCOPY);
  BitBltUgly(Canvas, xRightPanel, TopBarHeight + MapHeight - overlap, Panel.Width - xRightPanel,
    overlap, Panel.Canvas, xRightPanel, 0, SRCCOPY);
  BitBltUgly(Canvas, 0, TopBarHeight + MapHeight, Panel.Width, PanelHeight - overlap,
    Panel.Canvas, 0, overlap, SRCCOPY);
  if (pLogo >= 0) and (G.RO[pLogo] = nil) and (AILogo[pLogo] <> nil) then
    BitBltUgly(Canvas, xRightPanel + 10 - (16 + 64), ClientHeight - PanelHeight, 64, 64,
      AILogo[pLogo].Canvas, 0, 0, SRCCOPY);
end;
...
begin
  ...
  begin
    ...
    TButtonBase(Sender).Hint := Phrases.Lookup('CONTROLS', -1 + TButtonBase(
      Sender).Tag and $FF);
  end;
  SetMapOptions;
  MapValid := False;
...
procedure TMainScreen.FormKeyUp(Sender: TObject; var Key: word; Shift: TShiftState);
begin
