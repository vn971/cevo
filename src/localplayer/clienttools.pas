...
begin
  ...
  begin
    ...
    if ((MyRO.Government = gFuture) or (MyCity[cix].Size >= NeedAqueductSize) and
      (CityReport.FoodSurplus < 2)) and (CityReport.FoodSurplus > 0) then
  ...
  begin
    ...
    begin
      ...
      if MyCity[cix].Built[imPalace] > 0 then
        Inc(Flags, ciCapital);
      if (MyCity[cix].Built[imWalls] > 0) or (MyMap[MyCity[cix].Loc] and
        fGrWall <> 0) then
        Inc(Flags, ciWalled);
...
begin
  ...
  if (MyUn[uix].Movement > 0) or (MyRO.Wonder[woShinkansen].EffectiveOwner = me) then
    if (MyUn[uix].Movement >= 100) or ((MyModel[MyUn[uix].mix].Kind = mkCaravan) and
      ...
      for dx := -2 to 2 do
        for dy := -2 to 2 do
          if abs(dx) + abs(dy) = 2 then
            if Server(sMoveUnit - sExecute + dx and 7 shl 4 + dy and 7 shl
              7, me, uix, nil^) >= rExecuted then
              Result := False;
end;
...
begin
  ...
  for cix := 0 to MyRO.nCity - 1 do
    if (cix <> cixExcept) and (MyCity[cix].Loc >= 0) and
      (MyCity[cix].Flags and chCaptured = 0) and (Distance(MyCity[cix].Loc, Loc) <= 5) then
    begin
...
begin
  ...
  begin
    ...
    for uix1 := 0 to MyRO.nUn - 1 do
      if (MyUn[uix1].Loc >= 0) and (MyUn[uix1].Master = uix) and (MyUn[uix1].Home >= 0) then
