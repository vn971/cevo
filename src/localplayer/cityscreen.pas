...
type
  ...
  private
    ...
    cOwner, cGov, emix{enemy model index of produced unit},
...
begin
  ...
  begin
    ...
    BitBltUgly(Back.Canvas, 0, 0, ClientWidth, ClientHeight,
...
begin
  ...
  BitBltUgly(SmallCityMap.Canvas, 0, 0, 83, hSmallMap, SmallCityMapTemplate.Canvas,
    83 * SizeClass, 0, SRCCOPY);
  if IsPort then
  begin
    BitBltUgly(SmallCityMap.Canvas, 83, 0, 15, hSmallMap,
...
begin
  BitBltUgly(ZoomCityMap.canvas, 0, 0, wZoomMap, hZoomMap, Back.Canvas,
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        xGr := 66 + 22;
      BitBltTransparent(offscreen.Canvas, x + xxt - 5 + d * (2 * i + 1 - Total), y + yyt - 5, 10, 10, xGr, yGr, system1transparent);
    end;
  end;
...
var
  x, y, xGr, i, j, iix, d, dx, dy, PrCost, Cnt, Loc1, FreeSupp, Paintiix, HappyGain,
...
begin
  ...
  BitBltUgly(offscreen.canvas, 0, 0, 640, 480, Back.Canvas, 0, 0, SRCCOPY);
  ...
  begin
    ...
    RisedTextout(offscreen.canvas, 167 - BiColorTextWidth(offscreen.canvas, s) div
      2, ymOpt - 9, s);
  end
  ...
  begin
    ...
    BitBltTransparent(offscreen.Canvas, xmOpt - 32, ymOpt - 32, 64, 64,
      1 + OptiType mod 3 * 64, 217 + OptiType div 3 * 64, system2transparent);
  ...
  BitBltUgly(offscreen.canvas, xmArea + 102, 42, 90, 33, Back.Canvas,
  ...
  if IsCityAlive then
  begin
    CountBar(offscreen, xFood, yFood + dyBar div 2, wBar, 0, Phrases.Lookup(
      'FOOD'), Report.CollectedFood, MainTexture);
    CountBar(offscreen, xFood + dxBar, yFood + dyBar, wBar, 0, Phrases.Lookup(
      'DEMAND'), 2 * c.Size, MainTexture);
    CountBar(offscreen, xFood + dxBar, yFood, wBar, 0, Phrases.Lookup('SUPPORT'),
      Report.FoodSupport, MainTexture);
    if Report.FoodSurplus >= 0 then
      if (cGov = gFuture) or (c.Size >= NeedAqueductSize) and (Report.FoodSurplus < 2) then
        CountBar(offscreen, xFood + dxBar, yFood + 2 * dyBar, wBar, 6, Phrases.Lookup('PROFIT'),
          Report.FoodSurplus, MainTexture)
      else
        CountBar(offscreen, xFood + dxBar, yFood + 2 * dyBar, wBar, 0, Phrases.Lookup('SURPLUS'),
          Report.FoodSurplus, MainTexture)
    else
    begin
  ...
  CanGrow := (c.Size < MaxCitySize) and (cGov <> gFuture) and (Report.FoodSurplus > 0) and
    ((c.Size < NeedAqueductSize) or (c.Built[imAqueduct] = 1) and
    (c.Size < NeedSewerSize) or (c.Built[imSewer] = 1));
  FillBar(xFood + 3, yFood + 102, TrueFood,
    CutCityFoodSurplus(Report.FoodSurplus, IsCityAlive, cGov, c.size),
    Report.Storage, 1, CanGrow);
  LoweredTextOut(offscreen.Canvas, -1, MainTexture, xFood + 3 - 5, yFood + 102 -
    20, Format('%d/%d', [TrueFood, Report.Storage]));
  ...
  begin
    ...
    if Report.Production >= 0 then
      if c.Project and (cpImp + cpIndex) = cpImp + imTrGoods then
        ...
        CountBar(offscreen, xProd + dxBar, yProd + dyBar + 16, wBar, 2, Phrases.Lookup('PROD'),
          Report.Production, MainTexture)
    else
    begin
  ...
  begin
    CountBar(offscreen, xTrade, yTrade + dyBar div 2, wBar, 4, Phrases.Lookup(
      'TRADE'), Report.CollectedTrade, MainTexture);
    CountBar(offscreen, xTrade + dxBar, yTrade + 2 * dyBar, wBar, 5, Phrases.Lookup(
      'CORR'), Report.Corruption, MainTexture);
    CountBar(offscreen, xTrade + dxBar, yTrade, wBar, 6, Phrases.Lookup('TAX'),
      Report.Tax, MainTexture);
    CountBar(offscreen, xTrade + dxBar, yTrade + dyBar, wBar, 12, Phrases.Lookup(
      'SCIENCE'), Report.Science, MainTexture);
  end;

  // small map
  BitBltUgly(Offscreen.Canvas, xSmallMap, ySmallmap, wSmallMap, hSmallMap,
  ...
  Frame(Offscreen.Canvas, xSupport - 1, ySupport - 1, xSupport + wSupport, ySupport +
    hSupport, $B0B0B0, $FFFFFF);
  ...
  BitBltTransparent(offscreen.Canvas, x - 16, y - 5, 10, 10, 88, 115, system1transparent);
  ...
  BitBltUgly(Offscreen.Canvas, xZoomMap, yZoommap, wZoomMap, hZoomMap,
    ...
    for i := 0 to 5 do
      ...
      begin
        ...
        begin
          ...
          begin
            BitBltTransparent(offscreen.Canvas, x + 46, y, 14, 14, 82, 100, system1transparent);
          end
          else
          begin
            HappyGain := 0;
            ...
            for j := 0 to HappyGain - 1 do
              BitBltTransparent(offscreen.Canvas, x + 50, y + d * j, 10, 10, 132, 126, system1transparent);
          end;
          for j := 0 to Imp[iix].Maint - 1 do
            BitBltTransparent(offscreen.Canvas, x - 4, y + 29 - 3 * j, 10, 10, 132, 115, system1transparent);
        end;
      end;
    if imix[0] >= 0 then
  ...
  begin
    LoweredTextOut(offscreen.Canvas, -1, MainTexture, xZoomMap + 6, yZoomMap +
      2, Phrases.Lookup('SUPUNITS'));
        ...
        begin
          ...
          begin // unit visible in display
            ...
            for j := 0 to UnitReport.FoodSupport - 1 do
              BitBltTransparent(offscreen.Canvas, x + 38 + 11 * j, y + 40, 10, 10, 66, 115, system1transparent);
            for j := 0 to UnitReport.ProdSupport - 1 do
            begin
              if (FreeSupp > 0) and (UnitReport.ReportFlags and urfAlwaysSupport = 0) then
              begin
                BitBltTransparent(offscreen.Canvas, x + 16 - 11 * j, y + 40, 10, 10, 143, 115, system1transparent);
                Dec(FreeSupp);
              end
              else
                BitBltTransparent(offscreen.Canvas, x + 16 - 11 * j, y + 40, 10, 10, 88, 115, system1transparent);
            end;
            if UnitReport.ReportFlags and urfDeployed <> 0 then
              for j := 0 to 1 do
                BitBltTransparent(offscreen.Canvas, x + 27 + 11 * j, y + 40, 10, 10, 154, 126, system1transparent);
  ...
  begin
    ...
    begin // city type autobuild
      FrameImage(offscreen.canvas, improvementsPNG, xView + 9, yView + 5, xSizeBig, ySizeBig,
        (c.Status and 7 - 1 + 3) * xSizeBig, 0,
        (cix >= 0) and (ClientMode < scContact));
    end
    else if c.Project and cpImp = 0 then
    begin // project is unit
      FrameImage(offscreen.canvas, improvementsPNG, xView + 9, yView + 5, xSizeBig, ySizeBig, 0, 0,
...
begin
  ...
  begin {own city}
    ...
    ProdHint := (cGov <> gAnarchy) and
      (Happened and (chProduction or chFounded or chCaptured or chAllImpsMade) <> 0);
...
begin
  ...
  begin // own city
    ...
    while (cix >= 0) and (MyCity[cix].Loc <> Loc) do
      Dec(cix);
    assert(cix >= 0);
    if (Optimize_cixTileChange >= 0) and (Optimize_TilesBeforeChange and
      not MyCity[Optimize_cixTileChange].Tiles <> 0) then
    begin
...
{      MessgText:=Phrases.Lookup('OUTOFCONTROL');
    ...
    begin
      ...
      while (i >= 0) and not ((x >= xZoomMap + 14 + 72 * (i mod 3)) and
          (x < xZoomMap + 14 + 56 + 72 * (i mod 3)) and (y >= yZoomMap + 14 + 56 * (i div 3)) and
                ...
                begin
                  ...
                  begin
                    ...
                    if not Phrases2FallenBackToEnglish then
                      ...
                      MessgText := Format(Phrases.Lookup('DISPOSE'), [MessgText]);
                    if iix = imSpacePort then
                      with MyRO.Ship[me] do
                        if Parts[0] + Parts[1] + Parts[2] > 0 then
                          MessgText := MessgText + ' ' + Phrases.Lookup('SPDESTRUCTQUERY');
    ...
    begin
      ...
      while (i >= 0) and not ((x >= xZoomMap + 64 * (i mod 3)) and
          (x < xZoomMap + 64 + 64 * (i mod 3)) and (y >= yZoomMap + 20 + 48 * (i div 3)) and
    ...
    begin
      ...
      dx := qx - qy;
      dy := qx + qy;
      if (dx >= -3) and (dx <= 3) and (dy >= -3) and (dy <= 3) and (dx * dx * dy * dy < 81) and
        ((dx <> 0) or (dy <> 0)) then
        if ssShift in Shift then
        begin // terrain help
          Loc1 := dLoc(cLoc, dx, dy);
      ...
      (c.Flags and chCaptured = 0) and (x >= xmOpt - 32) and (x < xmOpt + 32) and
      (y >= ymOpt - 32) and (y < ymOpt + 32) then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin // loss of material -- do query
            ...
            else if MyCity[cix].Project and (cpImp or cpIndex) = MyCity[cix].Project0 and
              (cpImp or cpIndex) then
      ...
      if (NewProject and cpImp = 0) and ((MyCity[cix].Size < 4) and
        (MyModel[NewProject and cpIndex].Kind = mkSettler) or
        (MyCity[cix].Size < 3) and ((MyModel[NewProject and cpIndex].Kind = mkSlaves) or
...
begin
  ...
  begin
    if Optimize_TilesBeforeChange and not MyCity[Optimize_cixTileChange].Tiles <> 0 then
    begin
      CityOptimizer_ReleaseCityTiles(Optimize_cixTileChange,
        Optimize_TilesBeforeChange and not MyCity[Optimize_cixTileChange].Tiles);
      if WindowMode <> wmModal then
...
begin
  ...
  begin
    ...
    if BlinkTime = 0 then
      with Canvas do
      begin
        BitBltUgly(canvas, xView + 5, yView + 1, 64, 2,
          back.Canvas, xView + 5, yView + 1, SRCCOPY);
        BitBltUgly(canvas, xView + 5, yView + 3, 2, 42,
          back.Canvas, xView + 5, yView + 3, SRCCOPY);
        BitBltUgly(canvas, xView + 5 + 62, yView + 3, 2, 42,
    ...
    begin
      ...
      begin // city type autobuild
        FrameImage(canvas, improvementsPNG, xView + 9, yView + 5, xSizeBig, ySizeBig,
      ...
      begin // project is unit
        ...
        ImpImage(Canvas, xView + 9, yView + 5,
...
procedure TCityDlg.FormKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
begin
  if ((Key = VK_UP) or (Key = VK_NUMPAD8)) and (cix >= 0) and (WindowMode = wmPersistent) then
    ChangeCity(-1)
