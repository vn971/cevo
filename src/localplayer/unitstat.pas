...
begin
  ...
  begin
    ...
    BitBltUgly(Back.Canvas, 0, 0, wCommon, hOwnModel,
      MainTexture.Image.Canvas, (wMainTexture - wCommon) div 2,
      (hMainTexture - hOwnModel) div 2, SRCCOPY);
    BitBltUgly(Back.Canvas, wCommon, 0, wCommon, hEnemyModel,
      MainTexture.Image.Canvas, (wMainTexture - wCommon) div 2,
      (hMainTexture - hEnemyModel) div 2, SRCCOPY);
    BitBltUgly(Back.Canvas, 2 * wCommon, 0, wCommon, hEnemyUnit,
      MainTexture.Image.Canvas, (wMainTexture - wCommon) div 2,
      (hMainTexture - hEnemyUnit) div 2, SRCCOPY);
    BitBltUgly(Back.Canvas, 3 * wCommon, 0, wCommon, hEnemyCityDefense,
      MainTexture.Image.Canvas, (wMainTexture - wCommon) div 2,
      (hMainTexture - hEnemyCityDefense) div 2, SRCCOPY);
    BitBltUgly(Back.Canvas, 4 * wCommon, 0, wCommon, hEnemyCity,
      MainTexture.Image.Canvas, (wMainTexture - wCommon) div 2,
...
begin
  ...
  ConscriptsBtn.Visible := not supervising and (Kind = dkOwnModel) and
    (MyRO.Tech[adConscription] >= tsApplicable) and (MyModel[mixShow].Domain = dGround) and
    (MyModel[mixShow].Kind < mkScout);
  IsSpecialUnit := False;
...
var
  ...
  begin
    ...
    begin
      ...
      Result := (PPicture.HGr = PTestPicture.HGr) and (PPicture.pix = PTestPicture.pix) and
    ...
    with dst.Canvas do
      ...
      begin
        ...
        begin
          num := 0;
          case i of
            mcSeaTrans: if mi.Domain = dSea then
                num := mi.TTrans;
            mcCarrier: if mi.Domain = dSea then
                num := mi.ATrans_Fuel;
            mcBombs: num := mi.Bombs;
            mcFuel: if mi.Domain = dAir then
                num := mi.ATrans_Fuel;
            mcAirTrans: if mi.Domain = dAir then
                num := mi.TTrans;
...
var
  i, j, x, y, cix, uix, emix, InProd, Available, Destroyed, Loc, Cnt, yView, yTotal, yCaption: integer;
  s: string;
  ui: TUnitInfo;
  mi: TModelInfo;
begin
  ...
  case Kind of
    dkOwnModel:
    begin
      BitBltUgly(offscreen.canvas, 0, 0, wCommon, hOwnModel, Back.Canvas, 0, 0, SRCCOPY);
      yView := 13;
      yTotal := 92;
    end;
    dkEnemyModel:
    begin
      BitBltUgly(offscreen.canvas, 0, 0, wCommon, hEnemyModel, Back.Canvas,
        wCommon, 0, SRCCOPY);
      yView := 13;
      yTotal := 92;
    end;
    ...
    begin
      BitBltUgly(offscreen.canvas, 0, 0, wCommon, hEnemyUnit, Back.Canvas,
        2 * wCommon, 0, SRCCOPY);
      yView := 13;
      yTotal := 123;
    end;
    dkEnemyCityDefense:
    begin
      BitBltUgly(offscreen.canvas, 0, 0, wCommon, hEnemyCityDefense,
    ...
    dkEnemyCity:
    begin
      BitBltUgly(offscreen.canvas, 0, 0, wCommon, hEnemyCity, Back.Canvas,
        4 * wCommon, 0, SRCCOPY);
    end;
  end;
  ...
  begin // show city defense facilities
    ...
    for i := 0 to 3 do
      ...
      begin
        ...
        BitBltUgly(offscreen.Canvas, x, yImp, xSizeSmall, ySizeSmall,
    ...
    begin
      ...
      RFrame(offscreen.canvas,xView-2,yView-2,xView+65,yView+49,
        MainTexture.clBevelShade,MainTexture.clBevelLight);}
          ...
          BitBltUgly(offscreen.canvas, xView, yView + 16, 64, 32, Buffer.Canvas, 1, 0,
            SRCCOPY);
          ...
          if Flags and unFortified <> 0 then
            BitBltTransparent(offscreen.Canvas, xView, yView, xxu * 2, yyu * 2, 1 + 6 * (xxu * 2 + 1), 1, stdUnitsPng);
          ...
          begin
            ...
            RisedTextOut(offscreen.canvas, xView - 45 + 20 - BiColorTextWidth(
              offscreen.Canvas, s) div 2,
              yView + 23, s);
          end;
      ...
      RisedTextOut(offscreen.Canvas, xTotal + 170 - BiColorTextWidth(
        Offscreen.Canvas, s), yTotal, s);
      ...
      LoweredTextOut(offscreen.Canvas, -1, MainTexture, xTotal - 2, yTotal +
        57, Phrases.Lookup('UNITCOST'));
      ...
      begin
        ...
        begin
          ...
          RisedTextOut(offscreen.Canvas, xTotal + 170 - BiColorTextWidth(Offscreen.Canvas, s),
            (yTotal + StatDown - 19), s);
        end;

        NumberBar(offscreen, xTotal, yTotal + StatDown, Phrases.Lookup(
          'UNITBUILT'), MyModel[mixShow].Built, MainTexture);
        if MyModel[mixShow].Lost > 0 then
          NumberBar(offscreen, xTotal, yTotal + StatDown + 19, Phrases.Lookup(
            'UNITLOST'), MyModel[mixShow].Lost, MainTexture);
        if InProd > 0 then
          NumberBar(offscreen, xTotal, yTotal + StatDown + 57, Phrases.Lookup(
            'UNITINPROD'), InProd, MainTexture);
        if Available > 0 then
          NumberBar(offscreen, xTotal, yTotal + StatDown + 38, Phrases.Lookup(
            'UNITAVAILABLE'), Available, MainTexture);
      ...
      begin
        if Destroyed > 0 then
          NumberBar(offscreen, xTotal, yTotal + StatDown - 19, Phrases.Lookup(
            'UNITDESTROYED'), Destroyed, MainTexture);
        if Available > 0 then
          NumberBar(offscreen, xTotal, yTotal + StatDown, Phrases.Lookup(
            'UNITKNOWN'), Available, MainTexture);
      end;
    end;
  end;
  ...
  RisedTextOut(offscreen.Canvas,
    (ClientWidth - BiColorTextWidth(offscreen.Canvas, Caption)) div 2, yCaption, Caption);
end; {OffscreenPaint}
