...
begin
  ...
  ContactEnabled := ShowContact and not supervising and (1 shl pView and MyRO.Alive <> 0);
...
begin
  ...
  RisedTextout(offscreen.Canvas,
    40{(ClientWidth-BiColorTextWidth(offscreen.canvas,caption)) div 2}, 7, Caption);
  ...
  begin
    ...
    begin
      ...
      LoweredTextOut(Canvas, -2, MainTexture, 42, 445, Phrases2.Lookup('MENU_TELLAI'));
  end;
...
begin
  ...
  begin
    ...
    for p1 := 0 to nPl - 1 do
      if ExtinctPart and (G.Difficulty[p1] > 0) and (1 shl p1 and MyRO.Alive = 0) or
        not ExtinctPart and (1 shl p1 and MyRO.Alive <> 0) and (MyRO.Treaty[p1] >= trNone) then
      begin
        m := TMenuItem.Create(Popup);
        m.RadioItem := True;
...
procedure TNatStatDlg.FormKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
var
  i: integer;
begin
  ...
  begin // toggle nation
    ...
    repeat
      ...
      Inc(i);
    until (i >= nPl) or (1 shl pView and MyRO.Alive <> 0) and (MyRO.Treaty[pView] >= trNone);
    if i >= nPl then
      pView := me;
    Tag := pView;
