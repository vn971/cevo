...
function SimpleQuery(QueryKind: integer; SimpleText, SoundItem: string): integer;
...
begin
  ...
  begin
    if nLostArmy > LostUnitsPerLine * 6 then
      ...
      ClientHeight := ClientHeight + ((nLostArmy - 1) div LostUnitsPerLine + 1) * 48;
  end;
  case CenterTo of
    0:
    begin
...
begin
  ...
  // paint
  BitBltUgly(LogoBuffer.Canvas.Handle, 0, 0, wb, hb, ca.handle, x, y, SRCCOPY);
      ...
      for ix := 0 to wScrewed - 1 do
        if Screwed[ix, iy, 3] > 0.01 then
          LogoBuffer.Canvas.Pixels[xScrewed + ix, yScrewed + iy] :=
            trunc(Screwed[ix, iy, 2] / Screwed[ix, iy, 3]) + trunc(
            Screwed[ix, iy, 1] / Screwed[ix, iy, 3]) shl
            8 + trunc(Screwed[ix, iy, 0] / Screwed[ix, iy, 3]) shl 16;

  ImageOp_BCC(LogoBuffer, Templates, 0, 0, xb, yb, wb, hb, clCover, clPage);

  BitBltUgly(ca.handle, x, y, wb, hb, LogoBuffer.Canvas.Handle, 0, 0, SRCCOPY);
end;
...
begin
  ...
  case IconKind of
    ...
    mikEnemyShipComplete:
    begin
      BitBltUgly(Buffer.Canvas.Handle, 0, 0, 140, 120, Canvas.Handle,
    ...
    mikEnemyArmy:
      PaintEnemyArmy;
    mikFullControl:
      BitBltTransparent(Canvas, ClientWidth div 2 - 31, 24, 63, 63, 1, 281,system2transparent);
    mikShip:
