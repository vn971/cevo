        ...
        case Domain of {age determination}
          dGround:
            if (Attack >= Defense * 4) or (Attack > 0) and
              (MaxUpgrade < 10) and (Cap and (1 shl (mcArtillery - mcFirstNonCap)) <> 0) then
            begin
              Result := 170;
...
begin
  ...
  begin
    ...
    if (Length(Input) >= 3) and (Input[1] = '#') and (Input[2] in ['a'..'z']) and
      (Input[3] = ' ') then
...
begin
  ...
  begin
    Script.Insert(CityLine0 + nCityLines, Format(TPhrase('GENCITY'), [nCityLines + 1]));
    Inc(nCityLines);
  end;
