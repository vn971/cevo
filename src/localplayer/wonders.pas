...
begin
  ...
  Frame(Offscreen.Canvas, 1, 1, ClientWidth - 2, ClientHeight - 2, MainTexture.clBevelLight,
  ...
  RisedTextOut(Offscreen.Canvas, (ClientWidth - BiColorTextWidth(Offscreen.Canvas, s)) div
    2 - 1, 7, s);
  ...
  begin
    ...
    begin
      ...
      if (r < 8 * 128 * 180 * 180) and ((r >= 32 * 64 * 90 * 90) and (ax < amax2 * x) and
...
procedure TWondersDlg.FormMouseMove(Sender: TObject; Shift: TShiftState; X, Y: integer);
var
...
begin
  ...
  Selection := -1;
  for i := 0 to 20 do
    if (Imp[i].Preq <> preNA) and (x >= xm - xSizeBig div 2 + RingPosition[i, 0]) and
      (x < xm + xSizeBig div 2 + RingPosition[i, 0]) and (y >= ym - ySizeBig div
      2 + RingPosition[i, 1]) and (y < ym + ySizeBig div 2 + RingPosition[i, 1]) then
    begin
      Selection := i;
      break;
    end;
