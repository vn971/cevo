...
procedure TDraftDlg.OffscreenPaint;
  ...
  begin
    Result := (upgrade[d, 0].Preq = preNone) or (MyRO.Tech[upgrade[d, 0].Preq] >= tsApplicable);
  end;
  ...
  begin
    ...
    begin
      ...
      RisedTextOut(Offscreen.Canvas, xTotal2 + 170 + 64 + 30 - BiColorTextWidth(
        Offscreen.Canvas, s), y, s);
          ...
          begin
            ...
            begin
              ...
              BitBltTransparent(Offscreen.Canvas, xTotal2 - 1 - dx, y + 4, 10, 10,
                66 + i mod 11 * 11, 137 + i div 11 * 11, system1transparent);
              if num > 1 then
              begin
                s := IntToStr(num);
                w := TextWidth(s);
      ...
      BitBltTransparent(offscreen.Canvas, xTotal2 + 170 + 54 + 30, y + 4, 10, 10, 88, 115, system1transparent);
...
begin
  ...
  begin
    BitBltUgly(Back.Canvas, 0, 0, ClientWidth, 64, MainTexture.Image.Canvas,
      (wMainTexture - ClientWidth) div 2, hMainTexture - 64, SRCCOPY);
    BitBltUgly(Back.Canvas, 0, 64, ClientWidth, ClientHeight - 64,
      MainTexture.Image.Canvas, (wMainTexture - ClientWidth) div 2, 0, SRCCOPY);
  end
  else
    BitBltUgly(Back.Canvas, 0, 0, ClientWidth, ClientHeight, MainTexture.Image.Canvas,
  ...
  BitBltUgly(offscreen.canvas, 0, 0, ClientWidth, ClientHeight,
      ...
      for d := 0 to nDomains - 1 do
        ...
        begin
          ...
          if d = Domain then
            ...
            ImageOp_BCC(Offscreen, Templates, x, yDomain, 142, 246 + 37 * d, 36, 36, 0, $606060);
        end;
    ...
    with offscreen.Canvas do
      ...
      begin
        ...
        begin
          ...
          begin
            BitBltTransparent(offscreen.Canvas, xFeature - 9, yFeature + 2 + LinePitch * i,
              12, 12, 169, 185 + 13 * MyRO.DevModel.Cap[code[i]], system1transparent);
          ...
          LightGradient(offscreen.Canvas, xFeature + 34, yFeature + LinePitch * i, 50,
            system1transparent.Canvas.Pixels[187, 137]);
          ...
          for j := 0 to n - 1 do
            if j < CapWeight then
              BitBltTransparent(offscreen.Canvas, xFeature + 54 + (j * 2 + 1 - n) * d,
                yFeature + 2 + LinePitch * i + 1, 10, 10, 88, 126, system1transparent)
            else
              BitBltTransparent(offscreen.Canvas, xFeature + 54 + (j * 2 + 1 - n) * d,
                yFeature + 2 + LinePitch * i + 1, 10, 10, 88, 115, system1transparent);
        ...
        BitBltTransparent(offscreen.Canvas, xFeature + 20, yFeature + 2 + 1 + LinePitch * i,
          10, 10, 66 + code[i] mod 11 * 11, 137 + code[i] div 11 * 11, system1transparent);
  ...
  for i := 0 to nFeature - 1 do
    if (i in AutoFeature) and (1 shl Domain and Feature[i].Domains <> 0) and
      (Feature[i].Preq <> preNA) and ((Feature[i].Preq = preSun) and
      (MyRO.Wonder[woSun].EffectiveOwner = me) or (Feature[i].Preq >= 0) and
      (MyRO.Tech[Feature[i].Preq] >= tsApplicable)) and not
      ((Feature[i].Preq = adSteamEngine) and
    ...
    begin
      ...
      BitBltTransparent(offscreen.Canvas, xWeight + 7, yWeight + 36 + LinePitch * j,
        10, 10, 66 + i mod 11 * 11, 137 + i div 11 * 11, system1transparent);
...
begin
  Result := not (i in AutoFeature) and (1 shl d and Feature[i].Domains <> 0) and
    (Feature[i].Preq <> preNA) and ((Feature[i].Preq = preNone) or
    (Feature[i].Preq = preSun) and (MyRO.Wonder[woSun].EffectiveOwner = me) or
    (Feature[i].Preq >= 0) and (MyRO.Tech[Feature[i].Preq] >= tsApplicable));
end;
...
begin
  ...
  MaxLines := 0;
  for d := 0 to nDomains - 1 do
    if (upgrade[d, 0].Preq = preNone) or (MyRO.Tech[upgrade[d, 0].Preq] >= tsApplicable) then
    begin
      Count := 0;
      for i := 0 to nFeature - 1 do
  ...
  begin
    for d := 0 to nDomains - 1 do
      if (d <> Domain) and ((upgrade[d, 0].Preq = preNone) or
        ...
        (x >= xDomain + d * DomainPitch) and (x < xDomain + d * DomainPitch + 36) and
        (y >= yDomain) and (y < yDomain + 36) then
      begin
        SetDomain(d);
        Server(sCreateDevModel, me, Domain, nil^);
        MyModel[MyRO.nModel] := MyRO.DevModel;
    ...
    begin
      ...
      begin
        ...
        begin
          IncCap := code[i];
          BitBltTransparent(offscreen.Canvas, xFeature - 21, yFeature + 2 + LinePitch * i, 12, 12, 182, 172, system1transparent);
          SmartInvalidate;
        end
        ...
        begin
          DecCap := code[i];
          if code[i] < mcFirstNonCap then
            BitBltTransparent(offscreen.Canvas, xFeature - 9, yFeature + 2 + LinePitch * i, 12, 12, 182, 159, system1transparent)
          else
            BitBltTransparent(offscreen.Canvas, xFeature - 9, yFeature + 2 + LinePitch * i,
              12, 12, 182, 185 + 13 * MyRO.DevModel.Cap[code[i]], system1transparent);
          SmartInvalidate;
        end;
      end;
    end;
