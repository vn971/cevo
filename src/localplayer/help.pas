...
begin
  ...
  if THelpLineInfo(MainText.Objects[sb.si.npos + i]).Format in
    [pkCaption, pkBigTer, pkRightIcon, pkBigFeature] then
  begin
    ...
    for i := -sb.si.npos to InnerHeight div 24 do
      ...
      begin
        ...
        begin
          ...
          BitBltUgly(offscreen.Canvas, 8, 4 + i * 24, ExtPic.Width, yl,
        ...
        case HelpLineInfo.Format of
          ...
          begin
            ...
            if HelpLineInfo.Picpix = imPalace then
              BitBltUgly(offscreen.Canvas, 8 + x0[i], 2 + i * 24, xSizeSmall,
                ySizeSmall, SmallImp.Canvas,
                0 * xSizeSmall, 1 * ySizeSmall, SRCCOPY)
            else
              BitBltUgly(offscreen.Canvas, 8 + x0[i], 2 + i * 24, xSizeSmall, ySizeSmall,
                SmallImp.Canvas,
          ...
          begin
            if Imp[HelpLineInfo.Picpix].Kind <> ikWonder then
              ...
              WaterSign(InnerWidth - (40 + 2 * xSizeBig), i * 24 - 8, HelpLineInfo.Picpix + 7);
            x0[i] := x0[i] + 8;
          end;
          pkIllu:
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if i = 0 then
          i := 2;
        AddLine(Format(HelpText.Lookup('TECHFORMULA'),
          [TechFormula_M[i], TechFormula_D[i]]));
      end
      ...
      begin
        case no of
          ...
          begin
            ...
            LF;
            AddLine(HelpText.Lookup('HELPTITLE_TERLIST'), pkSpecialIcon, 1, hkTer, 200);
            LF;
            AddLine(HelpText.Lookup('HELPTITLE_JOBLIST'), pkSpecialIcon,
              2, hkMisc, miscJobList);
            LF;
            AddLine(HelpText.Lookup('HELPTITLE_TECHLIST'), pkBigIcon, 39, hkAdv, 200);
            LF;
            FindStdModelPicture(SpecialModelPictureCode[6], i, s);
            AddLine(HelpText.Lookup('HELPTITLE_MODELLIST'), pkModel, i, hkModel, 0);
            LF;
            AddLine(HelpText.Lookup('HELPTITLE_FEATURELIST'), pkBigIcon, 28, hkFeature, 200);
            LF;
            AddLine(HelpText.Lookup('HELPTITLE_IMPLIST'), pkBigIcon, 7 *
              SystemIconLines + imCourt, hkImp, 200);
            LF;
            ...
            LF;
            AddLine(HelpText.Lookup('HELPTITLE_KEYS'), pkBigIcon, 2,
              hkText, HelpText.GetHandle('HOTKEYS'));
            LF;
        ...
        begin // sigle terrain type
          ...
          begin
            ...
            if (no < 3 * 12) and (MineEff > 0) then
              MainText[Count - 1] :=
                MainText[Count - 1] + ' ' + Format(HelpText.Lookup('MOREMINE'), [MineEff]);
            ...
            if (no < 3 * 12) and (IrrEff > 0) then
              MainText[Count - 1] :=
                MainText[Count - 1] + ' ' + Format(HelpText.Lookup('MOREIRR'), [IrrEff]);
            ...
            begin
              ...
              for special := 1 to 2 do
                ...
                begin
                  ...
                  if i <> 0 then
                    MainText[Count - 1] :=
                      MainText[Count - 1] + Format(HelpText.Lookup('SPECIALFOOD'), [i]);
                  i := ProdRes[special] - ProdRes[0];
                  if i <> 0 then
                    MainText[Count - 1] :=
                      MainText[Count - 1] + Format(HelpText.Lookup('SPECIALPROD'), [i]);
                  i := TradeRes[special] - TradeRes[0];
                  if i <> 0 then
                    MainText[Count - 1] :=
                      MainText[Count - 1] + Format(HelpText.Lookup('SPECIALTRADE'), [i]);
                end;
            end;
            if no = 3 * 12 then
            begin
