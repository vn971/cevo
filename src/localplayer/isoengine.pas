...
begin
  ...
  begin
    ...
    for y := -2 to 6 do
      BitBltUgly(LandPatch.Canvas, (x + 2) * (xxt * 2),       (y + 2) * yyt, xxt, yyt, DitherMask.Canvas, xxt, yyt, SRCCOPY);
    for y := -2 to 6 do
      BitBltUgly(LandPatch.Canvas, (x + 2) * (xxt * 2) + xxt, (y + 2) * yyt, xxt, yyt, DitherMask.Canvas, 0, yyt, SRCCOPY);
  end;
  ...
  for x := 0 to 3 do
    ...
    begin
      ...
      BitBltUgly(OceanPatch.Canvas, x * (xxt * 2), y * yyt, xxt, yyt, DitherMask.Canvas, xxt, yyt, SRCAND);
      BitBltUgly(OceanPatch.Canvas, x * (xxt * 2) + xxt, y * yyt, xxt, yyt, DitherMask.Canvas, 0, yyt, SRCAND);
    end;
  ...
  for y := 0 to 3 do
    ...
    begin
      ...
      ySrc := 1 + yyt;
      if (x < 1) or (y >= 2) then
        BitBltTransparent(OceanMore.Canvas, x * (xxt * 2), y * yyt, xxt * 2, yyt, xSrc, ySrc, terrainCurrent);
      ...
      BitBltUgly(OceanMore.Canvas, x * (xxt * 2), y * yyt, xxt * 2, yyt, DitherMask.Canvas, 0, 0, SRCAND);
    end;

  BitBltUgly(DitherMask.Canvas, 0, 0, xxt * 2, yyt * 2, DitherMask.Canvas, 0, 0, DSTINVERT); {invert dither mask}
  BitBltTransparent(DitherMask.Canvas, 0, 0, xxt * 2, yyt * 2, 1, 1 + yyt, terrainCurrent);

  for x := -1 to 6 do
    for y := -2 to 6 do
      BitBltUgly(LandPatch.Canvas, (x + 2) * (xxt * 2), (y + 2) * yyt, xxt * 2, yyt, DitherMask.Canvas, 0, 0, SRCAND);

  for y := -1 to 6 do
    for x := -2 to 7 do
      BitBltUgly(LandMore.Canvas, (x + 2) * (xxt * 2) - xxt, (y + 2) * yyt, xxt * 2, yyt, DitherMask.Canvas, 0, yyt, SRCAND);

  BitBltUgly(LandPatch.Canvas, 0, 0, (xxt * 2) * 9, yyt * 9, LandMore.Canvas, 0, 0, SRCPAINT);

  for x := 0 to 3 do
    for y := 0 to 3 do
      BitBltUgly(OceanPatch.Canvas, x * (xxt * 2), y * yyt, xxt * 2, yyt, DitherMask.Canvas, 0, 0, SRCAND);

  for y := 0 to 3 do
    for x := 0 to 4 do
      BitBltUgly(OceanMore.Canvas, x * (xxt * 2) - xxt, y * yyt, xxt * 2, yyt, DitherMask.Canvas, 0, yyt, SRCAND);

  BitBltUgly(OceanPatch.Canvas, 0, 0, (xxt * 2) * 4, yyt * 4, OceanMore.Canvas, 0, 0, SRCPAINT);
  ...
  for x := 0 to 6 do
    BitBltUgly(LandPatch.Canvas, (x + 2) * (xxt * 2), yyt, xxt * 2, yyt, DitherMask.Canvas, 0, 0, SRCAND);

  BitBltUgly(DitherMask.Canvas, 0, 0, xxt * 2, yyt, DitherMask.Canvas, 0, 0, DSTINVERT);

  for y := 0 to 6 do
    BitBltUgly(LandPatch.Canvas, xxt * 2, (y + 2) * yyt, xxt * 2, yyt, DitherMask.Canvas, 0, 0, SRCAND);
...
begin
  ...
  BitBltUgly(FOutput.Canvas, x, y, Width, Height, Src.Canvas, xSrc,
    ySrc, Rop);
end;
...
begin
  ...
  if PureBlack then
    BitBltUgly(FOutput.Canvas, xDst, yDst, Width, Height, terrainCurrent.Canvas, xSrc, ySrc, SRCAND)
  else
...
begin
  ...
  BitBltTransparent(FOutput.Canvas, x + xxt div 2, y, xxt, yyt,
    1 + (Conn shr 6 + Conn and 1 shl 2) * (xxt * 2 + 1),
    1 + yyt + (16 + Tile and fTerrain) * (yyt * 3 + 1), terrainCurrent);
  BitBltTransparent(FOutput.Canvas, x + xxt, y + yyt div 2, xxt, yyt,
    1 + (Conn and 7) * (xxt * 2 + 1) + xxt,
    1 + yyt * 2 + (16 + Tile and fTerrain) * (yyt * 3 + 1), terrainCurrent);
  BitBltTransparent(FOutput.Canvas, x + xxt div 2, y + yyt, xxt, yyt,
    1 + (Conn shr 2 and 7) * (xxt * 2 + 1) + xxt,
    1 + yyt + (16 + Tile and fTerrain) * (yyt * 3 + 1), terrainCurrent);
  BitBltTransparent(FOutput.Canvas, x, y + yyt div 2, xxt, yyt,
        ...
        for dy := 0 to 1 do
          ...
          begin
            ...
            begin
              ...
              if p2 <> p1 then
              begin
                BitBltTransparent(FOutput.Canvas, x + dx * xxt, y + dy * yyt, xxt, yyt, 1 + 8 * (xxt * 2 + 1) + dx * xxt, 1 + yyt + 16 * (yyt * 3 + 1) + dy * yyt, terrainCurrent);
