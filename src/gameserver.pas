...
{$IFOPT O-}nHandoverStack,{$ENDIF}
  ...
  GWinner, GColdWarStart, GStealFrom, SpyMission, ZOCTile, CCCommand, CCPlayer: integer;
        ...
        begin
          ...
          if Copy(s, 1, 5) = '#NAME' then
            ...
            for i := 13 to Length(s) do
              case s[i] of
                '0'..'9': ServerVersion :=
                    ServerVersion and $FFFF00 + ServerVersion and
                    $FF * 10 + Ord(s[i]) - 48;
...
procedure ChangeClientWhenDone(Command, Player: integer; var Data; DataSize: integer);
begin
  CCCommand := Command;
  CCPlayer := Player;
  if DataSize > 0 then
...
var
  ...
  s: string[255];
begin
  MapFile := TFileStream.Create(UserDirectory + 'Maps' + DirectorySeparator + FileName, fmCreate or
    fmShareExclusive);
  MapFile.Position := 0;
  s := 'cEvoMap'#0;
...
begin
  ...
  MapFile := nil;
  try
    MapFile := TFileStream.Create(UserDirectory + 'Maps' + DirectorySeparator + FileName,
    ...
    begin
      ...
      begin
        RealMap[Loc1] := RealMap[Loc1] and ($7F01FFFF or fPrefStartPos or
          fStartPos) or ($F shl 27);
...
begin
  ...
  if nLocal <= 1 then
    for y := 0 to ly - 1 do
      ...
      begin
        ...
        SaveMap[x + lx * y] :=
          Tile and fTerrain + Tile and (fCity or fUnit or fOwned) shr 16;
      end;
  ...
  for i := 0 to nPl - 1 do
    ...
    begin
      ...
      s := '';
      {behavior} move(zero, s[Length(s) + 1], 4);
      LogFile.Write(s, (Length(s) div 4 + 1) * 4);
  ...
  for p := 0 to nPl - 1 do
    ...
    begin {initiate selected control module}
      ...
      begin // new game, this data version is original
        ...
        ProcessClientData[p] :=
          ProcessClientData[p] and (OriginalDataVersion[p] = Brain[bix[p]].DataVersion);
        ...
        if Un[uix].Loc >= 0 then
          ...
          begin
            if (Kind <= mkEnemyDeveloped) and (Un[uix].mix <> 1) then
              ...
              Inc(Stat[stMil, p, GTurn], (Attack + 2 * Defense) * Un[uix].Health div 100)
            else
              Inc(Stat[stMil, p, GTurn], (Attack + Defense) * Un[uix].Health div 100);
            case Kind of
...
begin
  ...
  if MovieMode then
    ...
    Mode := moLoading_Fast;
{$IFDEF TEXTLOG}
  AssignFile(TextLog, SavePath + LogFileName + '.txt');
  Rewrite(TextLog);
{$ENDIF}
  LoadOK := True;
  StartGame;
...
{$ENDIF}
    ...
    {$IFDEF TEXTLOG}
      WriteLn(TextLog, Format('Data Changes P%d (%d Bytes)',
        [p1, CL.State.LoadPos - LoadPos0]));
{$ENDIF}
    end
    else
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('Command %x', [Command]);
{$ENDIF}
...
begin
  ...
  begin
    ...
    {$IFDEF TEXTLOG}
      WriteLn(TextLog, 'AfterTurn - ExpandTerritory');
{$ENDIF}
    end
    else
    begin
  ...
  for cix := 0 to RW[p].nCity - 1 do
    with RW[p].City[cix] do
      if (Loc >= 0) and (Project and cpImp <> 0) and
        ((Project and cpIndex = woMIR) or (Imp[Project and cpIndex].Kind =
        ikShipPart)) then
      begin
...
begin
{$IFOPT O-}
  assert(1 shl pTurn and InvalidTreatyMap = 0);
{$ENDIF}
  ...
  if Mode > moLoading_Fast then
    MaskD(RW[pTurn].Map^, MapSize, not cardinal(fUnit or fHiddenUnit or
      fStealthUnit or fObserved or fSpiedOut or fOwned or fOwnZoCUnit or fInEnemyZoC));
  RW[pTurn].nEnemyUn := 0;
  ...
  if (Difficulty[pTurn] > 0) and (GWinner = 0) then
    ...
    begin
            ...
            for uix := 0 to nUn - 1 do
              with Un[uix] do
                if (Loc >= 0) and (Model[mix].Kind <> mkDiplomat) and
                  ((Home >= 0) or (Loc1 >= 0)) and (RealMap[Loc] shr
                  27 = cardinal(p1)) then
                begin
                  OldLoc := Loc;
                  if Master >= 0 then
            ...
            begin
              ...
              begin
                if Project0 and cpIndex = woMir then // MIR completed
                  ...
                  GColdWarStart := GTurn
                else if Imp[Project0 and cpIndex].Kind = ikShipPart then
                  {ship parts produced}
      ...
      if (Mode >= moMovie) and (GWinner = 0) and
        ((ShowShipChange.Ship1Change[0] > 0) or (ShowShipChange.Ship1Change[1] > 0) or
  ...
  begin
    // show observed areas
    if (GTestFlags and tfUncover <> 0) or (Difficulty[pTurn] = 0) then
      {supervisor - all tiles visible}
    begin
      if (bix[pTurn] <> bixNoTerm) and ((Difficulty[pTurn] > 0) or
        (Mode > moLoading_Fast)) then
        DiscoverAll(pTurn, lObserveSuper);
    end
    else
    begin
...
begin
  ...
  begin
    ...
    begin
      for p1 := 0 to nPl - 1 do
        if (1 shl p1 and GAlive <> 0) and ((p1 = pTurn) or
          (RW[pTurn].Treaty[p1] > trNoContact)) then
    ...
    for uix := 0 to nUn - 1 do
      ...
      begin
        ...
        if (Loc >= 0) and (Model[mix].Domain = dGround) and (Master < 0) and
          ((integer(Movement) = Model[mix].Speed) or (Model[mix].Cap[mcAcademy] > 0) and
          (Movement * 2 >= Model[mix].Speed)) then
    ...
    begin // restrict view area to current positions
      ...
      if Mode > moLoading_Fast then
        MaskD(RW[pTurn].Map^, MapSize, not cardinal(fUnit or fHiddenUnit or
          fStealthUnit or fObserved or fSpiedOut or fOwned or
          fOwnZoCUnit or fInEnemyZoC));
      RW[pTurn].nEnemyUn := 0;
      DiscoverViewAreas(pTurn);
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      for p1 := 0 to nPl - 1 do
        if (1 shl p1 and GWatching <> 0) and ((p1 <> p) or (bix[p1] = bixTerm)) then
        begin
...
{$ENDIF}
    ...
    begin
      if (PModel.Kind = mkDiplomat) or (PModel.Domain = dAir) or
        (PModel.Cap[mcRadar] + PModel.Cap[mcCarrier] + PModel.Cap[mcAcademy] > 0) or
        (RealMap[ToLoc] and fTerrain = fMountains) or
        (RealMap[ToLoc] and fTerImp = tiFort) or (RealMap[ToLoc] and
        fTerImp = tiBase) then
        ExtDiscover := True;
      if (PModel.Kind = mkDiplomat) or (PModel.Cap[mcSpy] > 0) then
        i := lObserveSuper
      else if (PModel.Domain = dAir) or (PModel.Cap[mcRadar] +
        PModel.Cap[mcCarrier] > 0) then
        i := lObserveAll
      else
        i := lObserveUnhidden;
      if ExtDiscover then
...
function ExecuteAttack(p, uix, ToLoc: integer; var MoveInfo: TMoveInfo;
  ...
  begin
    ...
    begin
      ...
      else if (nBattleHistory >= 16) and (nBattleHistory and
        (nBattleHistory - 1) = 0) then
...
{  if RW[p].Treaty[MoveInfo.Defender]=trCeaseFire then
    ...
    LoseCityPop := False;
    if (RealMap[ToLoc] and fCity <> 0) and ((MoveInfo.MoveType = mtAttack) and
      (MoveInfo.EndHealthDef <= 0) or (MoveInfo.MoveType = mtBombard) and
      (BombardmentDestroysCity or (RW[MoveInfo.Defender].City[MoveInfo.Dcix].Size > 2)))
    then
      case PModel.Domain of
        dGround: LoseCityPop :=
            (PModel.Cap[mcArtillery] > 0) or
            (RW[MoveInfo.Defender].City[MoveInfo.Dcix].Built[imWalls] = 0) and
            (Continent[ToLoc] <> GrWallContinent[MoveInfo.Defender]);
        dSea: LoseCityPop := RW[MoveInfo.Defender].City[MoveInfo.Dcix].Built
            [imCoastalFort] = 0;
        dAir: LoseCityPop := RW[MoveInfo.Defender].City[MoveInfo.Dcix].Built
            [imMissileBat] = 0;
      end;
    ...
    begin
      ...
      begin // update attacker
        ...
        Health := MoveInfo.EndHealth;
        Job := jNone;
        if RW[MoveInfo.Defender].Model[RW[MoveInfo.Defender].Un[
          MoveInfo.Duix].mix].Domain < dAir then
          Flags := Flags and not unBombsLoaded;
        Flags := Flags and not unFortified;
      ...
      begin // expel friendly unit
        ...
        begin
          ...
          for cix1 := 1 to nCity - 1 do
            if (City[cix1].Loc >= 0) and ((ExpelToLoc < 0) or
              (City[cix1].Built[imPalace] > 0)) then
    ...
    begin // destroy defenders
      if MoveInfo.MoveType <> mtBombard then
      begin
        ZoCDefenderDestroyed :=
          RW[MoveInfo.Defender].Model[RW[MoveInfo.Defender].Un[MoveInfo.Duix].mix].Flags
          and mdZOC <> 0;
...
begin
{$IFOPT O-}
  assert(1 shl p and InvalidTreatyMap = 0);
{$ENDIF}
  with RW[p].Un[uix] do
  begin
...
begin {>>>server}
  ...
  if (1 shl Player and (GAlive or GWatching) = 0) and not
    ((Command = sTurn) or (Command = sBreak) or (Command = sResign) or
  ...
  if (Mode = moPlaying) and not ((Command >= cClientEx) or (Command = sMessage) or
    (Command = sSetDebugMap) or (Command = sGetDebugMap) or
    (Command = sGetAIInfo) or (Command = sGetAICredits) or
    ...
    // no nego command
  begin
    PutMessage(1 shl 16 + 1, Format('No Turn: %d calls %x', [Player, Command shr 4]));
    Result := eNoTurn;
    exit;
  end;
  ...
  {sChangeSuperView:
    ...
    sGetGameChanged:
      if Player <> 0 then
        ...
        (CL.state.MoveCode = 0) and not HasCityTileChanges and not
        HasChanges(Player) then
        Result := eNotChanged;
    ...
    sGetHypoCityTileInfo:
      ...
      begin
        if (TTileInfo(Data).ExplCity < 0) or
          ...
          Result := GetTileInfo(Player, TTileInfo(Data).ExplCity,
            Subject, TTileInfo(Data));
      end
      else
        Result := eInvalid;
    ...
    sGetModels:
      if (GTestFlags and tfUncover <> 0) or (Difficulty[Player] = 0) then
      ...
      if (Subject >= 0) and (Subject < MapSize) and
        ...
        with TBattleForecast(Data) do
          if (1 shl pAtt and GAlive <> 0) and (mixAtt >= 0) and
            (mixAtt < RW[pAtt].nModel) and ((pAtt = Player) or
            (RWemix[Player, pAtt, mixAtt] >= 0)) then
          begin
            Result := GetBattleForecast(Subject, TBattleForecast(Data),
              uix1, cix1, AStr, DStr, ABaseDamage, DBaseDamage);
    ...
    begin
      ...
      {$ENDIF}
        ...
        until (pTurn = 0) or ((1 shl pTurn and (GAlive or GWatching) <> 0) and
            (GWinner = 0));
        if (Mode = moLoading_Fast) and ((GTurn = LoadTurn) or
          (GTurn = LoadTurn - 1) and (pTurn > 0)) then
          Mode := moLoading;
        if Mode = moPlaying then
        begin
          CCCommand := cTurn;
      ...
      {$IFDEF TEXTLOG}
        CmdInfo := CmdInfo + Format('---Turn %d P%d---', [GTurn, pTurn]);
{$ENDIF}
      end;
    end; // sTurn
      ...
      begin
        ...
        begin
          ...
          case Command of
            ...
            sNextRound: StartNewGame(SavePath, LogFileName, MapFileName,
                lx, ly, LandMass,
                MaxTurn);
    ...
    scDipBreak - sExecute:
      ...
      begin
        ...
        else if (Command and not sExecute = scDipCancelTreaty and not sExecute) and
          (RW[pDipActive].Treaty[p1] >= trPeace) then
        begin
          if (ServerVersion[pDipActive] >= $010100) and
            (GTurn < RW[pDipActive].LastCancelTreaty[p1] + CancelTreatyTurns) then
      ...
      if (pDipActive >= 0) and (LastEndClientCommand <> scDipCancelTreaty) and
        ...
        begin
          ...
          for i := 0 to TOffer(Data).nDeliver - 1 do
            if (TOffer(Data).Price[i] <> opChoose) and not
              PayPrice(pDipActive, p1, TOffer(Data).Price[i], False) then
              Result := eInvalidOffer;
          ...
          begin
            OfferFullySupported :=
              (TOffer(Data).nDeliver <= 2) and (TOffer(Data).nCost <= 2);
            // >2 no more allowed
            for i := 0 to TOffer(Data).nDeliver + TOffer(Data).nCost - 1 do
            begin
              if TOffer(Data).Price[i] and opMask = opTribute then
              ...
              if (TOffer(Data).Price[i] and opMask = opTreaty) and
                (TOffer(Data).Price[i] - opTreaty <= RW[pDipActive].Treaty[p1]) then
...

{
 ...
 ____________________________________________________________________
    ...
    sClearTestFlag:
      ...
      begin
      {$IFDEF TEXTLOG}
        CmdInfo := Format('ClearTestFlag %x', [Subject]);
{$ENDIF}
    ...
    sSetTestFlag:
      ...
      begin
      {$IFDEF TEXTLOG}
        CmdInfo := Format('SetTestFlag %x', [Subject]);
{$ENDIF}
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('SetGovernment P%d: %d', [Player, Subject]);
{$ENDIF}
      ...
      begin
        ...
        for p1 := 0 to nPl - 1 do
          if (p1 <> Player) and ((GAlive or GWatching) and (1 shl p1) <> 0) then
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('SetRates P%d: %d/%d', [Player, Subject and
        $f * 10, Subject shr 4 * 10]);
{$ENDIF}
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('Revolution P%d', [Player]);
{$ENDIF}
    ...
    sSetResearch, sSetResearch - sExecute: with RW[Player] do
      begin
    {$IFDEF TEXTLOG}
        CmdInfo := Format('SetResearch P%d: %d', [Player, Subject]);
{$ENDIF}
        if (Happened and phTech <> 0) and ((Subject < nAdv) or
          (Subject = adMilitary)) then
        begin
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('StealTech P%d: %d', [Player, Subject]);
{$ENDIF}
    ...
    {$IFDEF TEXTLOG}
      CmdInfo := Format('SetAttitude P%d to P%d: %d', [Player, p1, Subject]);
{$ENDIF}
      if (Subject >= nAttitude) or (p1 >= nPl) or (RW[Player].EnemyReport[p1] = nil) then
        Result := eInvalid
    ...
    sCancelTreaty, sCancelTreaty - sExecute:
      if (LastEndClientCommand <> scReject) or
        ...
        (GTurn < RW[Player].LastCancelTreaty[pContacted] + CancelTreatyTurns) then
        Result := eCancelTreatyRush
      else if Command >= sExecute then
      begin
...

{
 ...
 ____________________________________________________________________
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('CreateDevModel P%d', [Player]);
{$ENDIF}
      if Subject >= 4 then
        Result := eInvalid
          ...
          for i := 0 to nFeature - 1 do
            with Feature[i] do
              if (1 shl Domain and Domains <> 0) and
                ...
                (RW[Player].Tech[Preq] >= tsApplicable)) and (i in AutoFeature) then
                Cap[i] := 1;
          MaxWeight := 5;
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('SetDevModelCap P%d', [Player]);
{$ENDIF}
...

{
 ...
 ____________________________________________________________________
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('RemoveUnit P%d Mod%d Loc%d',
        [Player, RW[Player].Un[Subject].mix, RW[Player].Un[Subject].Loc]);
{$ENDIF}
      ...
      begin
        ...
        begin
          ...
          begin
            if (RW[Player].Model[RW[Player].Un[Subject].mix].Kind = mkCaravan) and
              ((Project and cpImp = 0) or (Imp[Project and cpIndex].Kind <>
              ikShipPart)) or (Project and cpImp = 0) and
            ...
            begin
              ...
              begin
                ...
                begin
                  Cost := integer(RW[Player].Model[mix].Cost) *
                    Health * BuildCostMod[Difficulty[Player]] div 1200;
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('SetUnitHome P%d Mod%d Loc%d',
        [Player, RW[Player].Un[Subject].mix, RW[Player].Un[Subject].Loc]);
{$ENDIF}
      if (Subject >= RW[Player].nUn) or (RW[Player].Un[Subject].Loc < 0) then
        Result := eInvalid
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('LoadUnit P%d Mod%d Loc%d',
        [Player, RW[Player].Un[Subject].mix, RW[Player].Un[Subject].Loc]);
{$ENDIF}
      if (Subject >= RW[Player].nUn) or (RW[Player].Un[Subject].Loc < 0) then
        Result := eInvalid
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('UnloadUnit P%d Mod%d Loc%d',
        [Player, RW[Player].Un[Subject].mix, RW[Player].Un[Subject].Loc]);
{$ENDIF}
      if (Subject >= RW[Player].nUn) or (RW[Player].Un[Subject].Loc < 0) then
        Result := eInvalid
    ...
    sCreateUnit - sExecute..sCreateUnit - sExecute + (nPl - 1) shl 4:
      ...
      begin
        ...
        if (Occupant[Loc1] >= 0) and (p1 <> Occupant[Loc1]) or
          (RealMap[Loc1] and fCity <> 0) and (RealMap[Loc1] shr 27 <> cardinal(p1)) or
          (RW[p1].Model[Subject].Domain < dAir) and
          ((RW[p1].Model[Subject].Domain = dSea) <>
    ...
    {$IFDEF TEXTLOG}
      CmdInfo := Format('MoveUnit P%d I%d Mod%d Loc%d (%d,%d)',
        [Player, Subject, RW[Player].Un[Subject].mix,
        RW[Player].Un[Subject].Loc, dx, dy]);
{$ENDIF}
      if (Subject >= RW[Player].nUn) or (RW[Player].Un[Subject].Loc < 0) then
        Result := eInvalid
      else
...

{
 ...
 ____________________________________________________________________
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('AddToCity P%d Mod%d Loc%d',
        [Player, RW[Player].Un[Subject].mix, RW[Player].Un[Subject].Loc]);
{$ENDIF}
      if (Subject >= RW[Player].nUn) or (RW[Player].Un[Subject].Loc < 0) then
        Result := eInvalid
      else if not (RW[Player].Model[RW[Player].Un[Subject].mix].Kind in
        [mkSettler, mkSlaves]) and (RW[Player].Un[Subject].Flags and
        unConscripts = 0) then
        Result := eViolation
      else
      begin
        ...
        begin
          ...
          with RW[Player].City[cix1] do
            ...
            begin {add to city}
              ...
              if (RW[Player].Model[RW[Player].Un[Subject].mix].Kind =
                mkSettler) and CanCityGrow(Player, cix1) then
                CityGrowth(Player, cix1);
              RemoveUnit_UpdateMap(Player, Subject);
            end;
        end;
    ...
    {$IFDEF TEXTLOG}
      CmdInfo := Format('StartJob P%d Mod%d Loc%d: %d',
        [Player, RW[Player].Un[Subject].mix, Loc0, i]);
{$ENDIF}
...

{
 ...
 ____________________________________________________________________
    ...
    {$IFDEF TEXTLOG}
      CmdInfo := Format('SetCityProject P%d Loc%d: %d',
        [Player, RW[Player].City[Subject].Loc, NewProject]);
{$ENDIF}
          ...
          begin
            ...
            begin
              if NewProject and cpIndex >= RW[Player].nModel then
                Result := eInvalid
              else if (NewProject and cpConscripts <> 0) and not
                ((RW[Player].Tech[adConscription] >= tsApplicable) and
                (RW[Player].Model[NewProject and cpIndex].Domain = dGround) and
                (RW[Player].Model[NewProject and cpIndex].Kind < mkScout)) then
                Result := eViolation;
            ...
            begin
              ...
              else if (NewProject = cpImp + imBank) and (Built[imMarket] = 0) or
                (NewProject = cpImp + imUniversity) and (Built[imLibrary] = 0) or
                (NewProject = cpImp + imResLab) and (Built[imUniversity] = 0) or
                (NewProject = cpImp + imMfgPlant) and (Built[imFactory] = 0) then
                Result := eNoPreq;
              ...
              if (GTestFlags and tfNoRareNeed = 0) and
                ...
                begin // check for rare resources
                  if NewProject and cpIndex = imShipComp then
                    ...
                    {if NewProject and cpIndex=imShipHab then} j := 3;
            ...
            begin
              if pt0 <> ptSelect then
                if NewProject and (cpImp or cpIndex) = Project0 and
                  ...
                  Prod := Prod0
                else if (pt1 = ptTrGoods) or (pt1 = ptShip) or
                  (pt1 <> pt0) and (pt0 <> ptCaravan) then
                begin
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('BuyCityProject P%d Loc%d',
        [Player, RW[Player].City[Subject].Loc]);
{$ENDIF}
      if (Subject >= RW[Player].nCity) or (RW[Player].City[Subject].Loc < 0) then
        Result := eInvalid
      else
        with RW[Player].City[Subject] do
          if (RW[Player].Government = gAnarchy) or (Flags and chCaptured <> 0) then
            ...
            ((Project and cpIndex = imTrGoods) or (Imp[Project and cpIndex].Kind =
            ikShipPart)) then
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('SellCityProject P%d Loc%d',
        [Player, RW[Player].City[Subject].Loc]);
{$ENDIF}
      if (Subject >= RW[Player].nCity) or (RW[Player].City[Subject].Loc < 0) then
        Result := eInvalid
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('SellCityImprovement P%d Loc%d: %d',
        [Player, RW[Player].City[Subject].Loc, integer(Data)]);
{$ENDIF}
      if (Subject >= RW[Player].nCity) or (RW[Player].City[Subject].Loc < 0) then
        Result := eInvalid
      else
    ...
    begin
      OldImp := integer(Data);
    {$IFDEF TEXTLOG}
      CmdInfo := Format('RebuildCityImprovement P%d Loc%d: %d',
        [Player, RW[Player].City[Subject].Loc, OldImp]);
{$ENDIF}
      if (Subject >= RW[Player].nCity) or (RW[Player].City[Subject].Loc < 0) then
        Result := eInvalid
      else
      begin
        if (OldImp < 0) or (OldImp >= nImp) or not
          (Imp[OldImp].Kind in [ikCommon, ikNatLocal, ikNatGlobal]) then
          Result := eInvalid
        else
          with RW[Player].City[Subject] do
            if (Built[OldImp] = 0) or (Project and cpImp = 0) or not
              (Imp[Project and cpIndex].Kind in [ikCommon, ikNatLocal, ikNatGlobal]) then
              Result := eInvalid
            ...
            begin
              Inc(Prod, Imp[OldImp].Cost * BuildCostMod[Difficulty[Player]] div
                12 * 2 div 3);
              Project0 := Project0 and not cpCompleted;
              if Project0 and not cpAuto <> Project and not cpAuto then
                Project0 := Project;
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('SetCityTiles P%d Loc%d: %x',
        [Player, RW[Player].City[Subject].Loc, integer(Data)]);
{$ENDIF}
      if (Subject >= RW[Player].nCity) or (RW[Player].City[Subject].Loc < 0) then
        Result := eInvalid
      else
        Result := SetCityTiles(Player, Subject, integer(Data), Command < sExecute);
    end;

{
 ...
 ____________________________________________________________________
      ...
      begin
      {$IFDEF TEXTLOG}
        CmdInfo := Format('ClientEx%x P%d', [Command, Player]);
{$ENDIF}
  ...
  begin
    ...
    begin
    {$IFDEF TEXTLOG}
      CmdInfo := Format('***ERROR (%x) ', [Result]) + CmdInfo;
{$ENDIF}
      LoadOK := False;
    end
    else
  ...
  if (Command and (cClientEx or sExecute or sctMask) = sExecute or sctEndClient) and
    (Result >= rExecuted) then
