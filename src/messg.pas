...
uses
  ...
  Messages, Classes, Graphics, Controls, Forms, ButtonBase, ButtonA,
  Area;
...
begin
  ...
  begin
    if integer(Hi(Msg.lParam)) >= Top + TitleHeight then
      Msg.Result := HTCLIENT
    else
    begin
      for i := 0 to ControlCount - 1 do
        ...
        begin
          ...
          if (integer(Lo(Msg.lParam)) >= Left + ControlBounds.Left) and
            (integer(Lo(Msg.lParam)) < Left + ControlBounds.Right) and
            (integer(Hi(Msg.lParam)) >= Top + ControlBounds.Top) and
            (integer(Hi(Msg.lParam)) < Top + ControlBounds.Bottom) then
          begin
            Msg.Result := HTCLIENT;
            exit;
          end;
  ...
  begin
    ...
    begin
      ...
      LoweredTextOut(Canvas, -1, MainTexture,
        (ClientWidth - BiColorTextWidth(Canvas, s)) div 2, 19 +
        Border + TopSpace + LinesCount * MessageLineSpacing, s);
    end;
    Start := Stop + 2;
    Inc(LinesCount);
  end;
