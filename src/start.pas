...
DefaultWorldTiles=4200;}
  ...
  InitAlive: array[1..nPl] of integer =
    (1, 1 + 2, 1 + 2 + 32, 1 + 2 + 8 + 128, 1 + 2 + 8 + 32 + 128, 1 +
    2 + 8 + 16 + 64 + 128, 1 + 2 + 4 + 16 + 32 + 64 + 256,
...
begin
  ...
  begin
    ...
    //DeleteObject(r1);
    r1 := CreateRectRgn(QuitBtn.left, QuitBtn.Top, QuitBtn.left +
      QuitBtn.Width, QuitBtn.Top + QuitBtn.Height);
...
const
  ...
  begin
    ...
    BitBltUgly(LogoBuffer.Canvas, 0, 0, 50, 50, Canvas,
      xActionIcon - 2, y - 2, SRCCOPY);
    GlowFrame(LogoBuffer, 8, 8, 34, 34, $202020);
    BitBltUgly(Canvas, xActionIcon - 2, y - 2, 50, 50, LogoBuffer.Canvas,
      0, 0, SRCCOPY);
...
begin
  ...
  begin
    ...
    if AutoDiff > 0 then
    begin
      Frame(Canvas, -1{x0Brain-dxBrain}, yMain + 112 -
        15{Up1Btn.Top-12}{y0Brain-dyBrain},
        x0Brain + dxBrain + 64,
        Up2Btn.Top + 38{y0Brain+dyBrain+64}, MainTexture.clBevelShade,
  ...
  // draw tabs
  Frame(Canvas, 2, 2 + 2 * integer(Tab <> 0), TabOffset + (0 + 1) *
    TabSize - 1, TabHeight, MainTexture.clBevelLight, MainTexture.clBevelShade);
  Frame(Canvas, 1, 1 + 2 * integer(Tab <> 0), TabOffset + (0 + 1) *
    TabSize, TabHeight, MainTexture.clBevelLight, MainTexture.clBevelShade);
  ...
  BitBltUgly(LogoBuffer.Canvas, 0, 0, 36, 36, Canvas, 6,
  ...
  // logo part 2
  BitBltUgly(Canvas, 6, 3 + 2 * integer(Tab <> 0), 36, 36, LogoBuffer.Canvas, 0, 0, SRCCOPY);
  ...
  begin
    ...
    begin
      ...
      for i := 0 to nPlOffered - 1 do
        ...
        begin
          if bixView[i] >= 0 then
            FrameImage(Canvas, BrainPicture[bixView[i]], xBrain[i],
              yBrain[i], 64, 64, 0, 0, True)
          else
          ...
          begin
            ...
            Frame(Canvas, xBrain[i] - 19, yBrain[i] + 18, xBrain[i] -
              ...
              $000000, $000000);
            RFrame(Canvas, DiffUpBtn[i].Left - 1, DiffUpBtn[i].Top -
              1, DiffUpBtn[i].Left + 12,
              DiffUpBtn[i].Top + 24, MainTexture.clBevelShade, MainTexture.clBevelLight);
            with Canvas do
            begin
              Brush.Color := $000000;
              FillRect(Rect(xBrain[i] - 5, yBrain[i] + 25, xBrain[i] -
                2, yBrain[i] + 27));
              Brush.Style := bsClear;
            end;
            if i in OfferMultiple then
            begin
              RFrame(Canvas, MultiBtn[i].Left - 1, MultiBtn[i].Top -
          ...
          begin
            DiffUpBtn[i].Hint :=
              Format(Phrases.Lookup('STARTCONTROLS', 9), [Brain[bixView[i]].Name]);
    ...
    begin
      ...
      DLine(Canvas, 24, xDefault - 6, yMain + 164 + 19, MainTexture.clBevelLight,
        MainTexture.clBevelShade);
      RisedTextOut(Canvas, 24{x0Brain+32-BiColorTextWidth(Canvas,s) div 2},
        yMain + 164{y0Mini-77},
        Phrases.Lookup('STARTCONTROLS', 16));
      if AutoDiff = 1 then
        FrameImage(Canvas, BrainPicture[bixBeginner], xDefault, yDefault,
          64, 64, 0, 0, False)
      else
        FrameImage(Canvas, BrainPicture[bixDefault], xDefault, yDefault,
          64, 64, 0, 0, True);
  ...
  begin
    ...
    Frame(Canvas, xMini, yMini, xMini + 3 + MiniWidth * 2, yMini + 3 + MiniHeight,
      MainTexture.clBevelLight,
      MainTexture.clBevelShade);
  ...
  begin
    BitBltUgly(Canvas, xMini + 2, yMini + 2, MiniWidth * 2, MiniHeight,
      Mini.Canvas, 0, 0, SRCCOPY);
    if page = pgStartRandom then
          ...
          if AutoDiff < 0 then
            ...
            begin
              if bixView[i] = -1 then
                ...
                Reg.WriteString('Control' + IntToStr(i), Brain[bixView[i]].FileName);
...
begin
  case Page of
    ...
    begin
      ...
      begin
        LoadTurn := LastTurn;
        SmartInvalidate(xTurnSlider - 2, y0Mini + 61, xTurnSlider +
          wTurnSlider + 2, yTurnSlider + 9);
      end;
      TurnValid := True;
    end;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          begin
            if (MapRow[x] and fTerrain) in [fGrass,
              fPrairie, fTundra, fSwamp, fForest, fHills] then
              Inc(nMapLandTiles);
    ...
    begin // turn all local players off
      for i := 1 to nPlOffered - 1 do
        ...
        begin
          ...
          SmartInvalidate(xBrain[i] - 31, yBrain[i] - 1, xBrain[i] +
            64, DiffUpBtn[i].Top + 25);
        end;
...
begin
  ...
  begin
    ...
    begin
      ...
      for i := bixFirstAI to nBrain - 1 do // offer available AIs
        if (Brain[i].Flags and fMultiple <> 0) or (Brain[i].Flags and fUsed = 0) or
          (i = bixView[bixPopup]) then
          OfferBrain(i);
    end;
  end;
end;
  ...
  for i := 0 to nPlOffered - 1 do
    ...
    begin
      if Sender = DiffUpBtn[i] then
        ...
        Dec(Difficulty[i]);
      SmartInvalidate(xBrain[i] - 18, yBrain[i] + 19, xBrain[i] - 18 +
        12, yBrain[i] + (19 + 14));
    end;
end;
...
procedure TStartDlg.FormKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
begin
...
procedure TStartDlg.FormMouseMove(Sender: TObject; Shift: TShiftState; X, Y: integer);
var
    ...
    begin
      if SelectedAction >= 0 then
        SmartInvalidate(ActionSideBorder, yAction + SelectedAction * ActionPitch - 8,
          ClientWidth - ActionSideBorder, yAction + (SelectedAction + 1) *
          ActionPitch - 8);
