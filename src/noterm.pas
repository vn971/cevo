...
begin
  ...
  BitBltUgly(Canvas.Handle, 64, 287 + 138, 192, 20, State.Canvas.Handle, 0, 0, SRCCOPY);
end;
...
{$ELSE}
        (now - LastShowYearTime)
{$ENDIF}
        / PerfFreq >= UpdateInterval then
      begin
        ShowYear;
      ...
      TurnTime :=
{$IFDEF VER100}
        (now.LowPart - LastNewTurn.LowPart)
{$ELSE}
        (now - LastNewTurn)
{$ENDIF}
        / PerfFreq;
    ...
    begin
      ...
      begin
        ActiveDuration :=
{$IFDEF VER100}
          (now.LowPart - LastShowTurnChange.LowPart)
{$ELSE}
          (now - LastShowTurnChange)
{$ENDIF}
          / PerfFreq;
        ...
        if not DisallowShowActive[Active] then
          ShowActive(Active, False);
        DisallowShowActive[Active] :=
          (ActiveDuration < TurnTime * 0.25) and (ActiveDuration < ShowActiveThreshold);
      end;
...
begin
  ...
  for i := 1 to nPlOffered - 1 do
    if bixView[i] >= 0 then
    begin
      Frame(Canvas, xBrain[i] - 24, yBrain[i] - 8 - 16, xBrain[i] -
        24 + 111, yBrain[i] - 8 - 16 + 111,
      ...
      if 1 shl i and G.RO[me].Alive = 0 then
        BitBltUgly(Canvas.Handle, xBrain[i], yBrain[i] - 16, 64, 64,
...
procedure TNoTermDlg.FormKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
begin
