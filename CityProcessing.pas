...
begin
  with RW[p].City[cix] do
    Result := (Size < MaxCitySize) and ((Size < NeedAqueductSize) or
...
begin
  ...
  begin
    ...
    if RW[p].Government = gFundamentalism then
      ...
      Inc(ScienceDoubling);
    FlexibleLuxury :=
      ((ServerVersion[p] >= $0100F1) and (GWonder[woLiberty].EffectiveOwner = p) or
      (ServerVersion[p] < $0100F1) and (GWonder[woMich].EffectiveOwner = p)) and
...
begin
  ...
  begin
    ...
    if plus > 0 then
    begin
      Lux := (4 * plus + 3 + TradeProcessing.LuxBonus) div
        (4 + TradeProcessing.LuxBonus);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Project0 and (not cpAuto or cpRepeat) = Project and not
          cpAuto or cpRepeat then
          Result := Result shr 1
        else
          Result := Result * 2;
    end
...
{$ENDIF}
      ...
      DetermineCityTradeProcessing(p, cix, BaseHappiness + Control - 2 * Deployed,
        TradeProcessing);
      ...
      if (GTestFlags and tfNoRareNeed = 0) and (ProdRep > Support) and
        ...
        (RareOK[1] = 0) or (Project and cpIndex = imShipPow) and
        (RareOK[2] = 0) or (Project and cpIndex = imShipHab) and (RareOK[3] = 0)) then
        ProdRep := Support;
    end;
  end;
  Result := eOk;
    ...
    for V21 := 1 to 26 do
      ...
      begin
        ...
        if (Loc1 >= 0) and (Loc1 < MapSize) and (RealMap[Loc1] and
          fTerrain >= fGrass) and (RealMap[Loc1] and
          (fPoll or fDeadLands or fCity) = 0) then
        begin
...
begin
  with RW[p], City[cix] do
    for i := 28 to nImp - 1 do
      if (Built[i] > 0) and (Project0 and (cpImp or cpIndex) <> (cpImp or i)) then
        // don't pay maintenance when just completed
      begin
...
begin
  with RW[p], City[cix], CityReport do
    ...
    begin
      ...
      if not Disorder and ((Government = gFuture) or
        (Size >= NeedAqueductSize) and (FoodSurplus < 2)) and (FoodSurplus > 0) then
      ...
      begin {calculate new food storage}
        ...
        if ((GTestFlags and tfImmGrow <> 0) or (Food >= CityStorage) and
          ...
          and (Size < MaxCitySize) and (Project and
          (cpImp + cpIndex) <> cpImp + imAqueduct) and (Project and
          (cpImp + cpIndex) <> cpImp + imSewer) and not CanCityGrow(p, cix) then
...
begin
  ...
  begin
    ...
    CheckGrow := (Flags and chDisorder = 0) and IsActive and (Government <> gFuture);
    ...
    begin
      ...
      for i := 0 to nUn - 1 do
        if (Un[i].Loc >= 0) and (Un[i].Home = cix) and
          ...
          {and (GWonder[woFreeSettlers].EffectiveOwner<>p)} or
          (Un[i].Flags and unConscripts <> 0)) and
    ...
    if DoProd and (Project and (cpImp or cpDisbandCity) = 0) and
      ((Size + SizeMod - 2 < 2) and (Model[Project and cpIndex].Kind = mkSettler) or
      (Size + SizeMod - 1 < 2) and ((Model[Project and cpIndex].Kind = mkSlaves) or
...
var
  ...
  WantedProd, MinFood, MinProd, Count, Take, MaxTake, AreaSize,
  FormulaCode, NeedRare, RareTiles, cix1, dx, dy, BestTiles, ProdBeforeBoost,
  TestTiles, SubPlus, SuperPlus: integer;
...
begin
  ...
  begin //decode evaluation formula from weights parameter
    ...
    if FormulaCode and $40 <> 0 then
      ValueFormula_Weight[i] :=
        (FormulaCode and $0F) * (1 shl (FormulaCode and $30 shr 4)) / 16
    else
      ValueFormula_Weight[i] :=
        (FormulaCode and $0F) * (1 shl (FormulaCode and $30 shr 4));
  end;
  ...
  begin
    ...
    begin
      ...
      if MinWorking > Size then
        MinWorking := Size;
      if (RW[p].LuxRate = 0) and not CityReportEx.TradeProcessing.FlexibleLuxury then
    ...
    if TestReport.Support = 0 then
      ...
      WantedProd := 1 + (TestReport.Support * 100 - 1) div
        (100 + CityReportEx.ProdProcessing.ProdBonus * 50 +
    ...
    begin
      ...
      begin
        Loc1 := Radius[V21];
        if (Loc1 >= 0) and (Loc1 < MapSize) and (RealMap[Loc1] and
          fModern = cardinal(NeedRare)) then
    ...
    for V21 := 1 to 26 do // non-rare tiles
      ...
      begin
        ...
        begin
          ...
          begin
            iT := 0;
            while (iT < nTile[iH]) and (TileInfo.Food <=
                Hierarchy[iH, iT].Food) and (TileInfo.Prod <= Hierarchy[iH, iT].Prod) and
              (TileInfo.Trade <= Hierarchy[iH, iT].Trade) and not
              ((TileInfo.Food = Hierarchy[iH, iT].Food) and
          ...
          move(Hierarchy[iH, iT], Hierarchy[iH, iT + 1], (nTile[iH] - iT) *
            sizeof(TTileData));
          Inc(nTile[iH]);
    ...
    repeat
      ...
      while (TestReport.Working < MaxWorking) and (iH < iH_Switch) and
        ((TestReport.Working < MinWorking) or (TestReport.FoodRep < TestReport.Eaten) or
      ...
      begin
        ...
        begin
          ...
          while (Take < MaxTake) and (TestReport.FoodRep +
              Hierarchy[iH, Take].Food < MinFood) do
            Inc(Take);
          while (Take < MaxTake) and (ProdBeforeBoost +
              Hierarchy[iH, Take].Prod < MinProd) do
            Inc(Take);
        end;
        nSelection[iH] := Take;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                BestTiles := TestTiles;
                TestReport.Happy :=
                  (CityReportEx.TradeProcessing.HappyBase - Size) div
                  2 + TestReport.Lux shr 1;
